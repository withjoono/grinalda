...
export const Route = createLazyFileRoute("/ms/_layout/evaluation-list")({
  component: SusiEvaluationList,
});

function SusiEvaluationList() {
  // Queries
  ...
  const filteredEvaluationList = useMemo(() => {
    ...
    // 타입 변환: currentUser.id는 string, officerId는 number
    const currentUserId = Number(currentUser.id);
    return evaluationList.filter((n) => n.officerId !== currentUserId);
              ...
              >
                ...
                <div className="flex w-full flex-col gap-y-2">
                  <p className="h-4 text-center text-sm text-primary">
                    {!isCompleted
                      ? `대기 순위: ${evaluation.remainingEvaluations}`
                      : ""}
                  </p>
                  <Button
