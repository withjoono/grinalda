...
// 토큰 함수들 - token-manager와 Zustand auth-storage 모두 지원
export const getAuthToken = () => {
  ...
  // 2. Zustand persist 스토리지에서 확인 (fallback)
...
export const setAuthToken = (token: string) => {
  ...
  // 2. Zustand persist 스토리지에도 저장 (fallback 호환성)
...
// 이 목록의 API들은 401 에러 시에도 로그인 페이지로 리다이렉트하지 않음
const PUBLIC_ENDPOINTS = [
  '/auth/me', // 인증 체크용 - 미로그인 시 null 반환해야 함 (리다이렉트 안함)
  '/auth/login/email',
  '/auth/login/oauth2',
  '/auth/register/email',
