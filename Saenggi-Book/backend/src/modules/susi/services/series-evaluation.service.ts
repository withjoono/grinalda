...
export class SeriesEvaluationService {
  ...
  async getUniversityLevel(universityName: string) {
    const university = await this.prisma.hub27UnivLevel.findFirst({
      where: { univ_name: universityName },
    });
    ...
    return {
      ...
      level: university.univ_level,
      universityCode: university.univ_code,
  ...
  async getAllUniversities() {
    const rows = await this.prisma.hub27UnivLevel.findMany({
      orderBy: { univ_name: 'asc' },
    });
    return rows.map((r) => ({
      universityName: r.univ_name,
      level: r.univ_level,
      universityCode: r.univ_code,
      region: r.region,
    }));
  }
