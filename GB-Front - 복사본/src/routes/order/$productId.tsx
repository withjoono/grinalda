...
import {
  useContractFreeService,
  usePreRegisterPayment,
  useValidCoupon,
  useVerifyPayment,
} from "@/stores/server/features/payments/mutations";
import { useGetStoreCode } from "@/stores/server/features/payments/queries";
import { useGetProduct } from "@/stores/server/features/products/queries";
...
function ProductOrder() {
  ...
  const storeCode = useGetStoreCode();
  const queryClient = useQueryClient();

  // mutations
  const preRegisterPayment = usePreRegisterPayment();
  const contractFreeService = useContractFreeService();
  const verifyPayment = useVerifyPayment();
  const validCoupon = useValidCoupon();
  ...
  const requestPay = async () => {
    ...
    if (!product.data) {
      toast.error("상품이 잘못되었습니다.");
      return;
    }
    if (!storeCode.data) {
      toast.error("상점 코드가 잘못되었습니다.");
      return;
    }
    const finalAmount = parseInt(product.data.productPrice) - discountAmount;

    // 결제할 값이 없으면 무료계약 체결
    if (finalAmount === 0) {
      const result = await contractFreeService.mutateAsync({
    ...
    // 결제 사전등록
    const preRegisterResult = await preRegisterPayment.mutateAsync({
      productId: product.data.id,
      couponNumber: couponNumber.length ? couponNumber : undefined,
    });
    if (!preRegisterResult.success) {
      toast.error(preRegisterResult.error);
      return;
    }
    const merchantUID = preRegisterResult.data.merchantUid;

    const IMP = window.IMP;
    IMP.init(import.meta.env.VITE_IMP_MERCHANT_ID);

    IMP.request_pay(
      {
        pg: "html5_inicis." + storeCode.data.storeCode,
        pay_method: "card",
        merchant_uid: merchantUID, // 주문 고유 번호
        name: product.data.productNm,
        amount: finalAmount,
        buyer_id: user.data.id,
        buyer_email: user.data.email,
        buyer_name: user.data.nickname,
        buyer_tel: user.data.phone,
      },
      async function (response: { success: boolean; error_msg?: string; imp_uid: string }) {
        try {
          const { success, error_msg, imp_uid } = response;

          if (!success) {
            toast.error(error_msg || "결제에 실패했습니다.");
            return;
          }
          ...
          /* eslint-disable */ console.log(
            ...oo_oo(`299420026_132_10_132_24_4`, e),
          );
  ...
  // 상품 타입 TEXT 변환
  const productTypeCodeName = (code: string) => {
    if (code === "FIXEDTERM") {
      return "기간권";
    } else if (code === "TICKET") {
      return "횟수권";
  ...
  const handleCouponSubmit = async () => {
    ...
    if (_couponNumber.length === 0) {
      toast.error("쿠폰번호를 입력해주세요");
      return;
    }

    const result = await validCoupon.mutateAsync({
      couponNumber: _couponNumber,
      productId: product.data.id,
    });
    if (result.success) {
      toast.success("성공적으로 쿠폰을 등록했습니다.");
      setDiscountAmount(result.data.discountPrice);
      setCouponInfo(result.data.couponInfo);
  ...
  const handleSuccess = async () => {
    ...
    // 사용자 정보와 활성 서비스 목록 즉시 갱신
    await Promise.all([
      user.refetch(),
      queryClient.refetchQueries({ queryKey: meQueryKeys.activeServices(), type: 'all' }),
    ]);
  ...
  return (
    <div className="mx-auto w-full px-4 py-12 sm:max-w-screen-sm">
      <h2 className="text-3xl font-semibold">주문/결제</h2>
      <div className="w-full space-y-8 pt-4">
        <div className="w-full space-y-2 p-4 py-8">
          <h3 className="text-2xl font-semibold">상품명</h3>
          <div className="flex w-full items-center justify-between">
            <p className="text-xl">{product.data.productNm}</p>
            <p className="text-xl">
              {formatPrice(Number(product.data.productPrice))}
            </p>
          </div>
            ...
            {couponInfo ? (
              ...
              <Dialog onOpenChange={setOpenCouponModal} open={openCouponModal}>
                <DialogTrigger asChild>
                  ...
                  >
                    <IconPlus className="mr-2 size-4" /> 쿠폰등록
                  </Button>
                </DialogTrigger>
          ...
          <div className="space-y-2 pt-4">
            <p>{product.data.refundPolicy}</p>
            <p>
              결제일로부터 서비스 미사용시 14일 이내 환불 받으실 수 있습니다.
            </p>
            <p>기타 문의 및 영수증 발급은 고객센터를 이용해 주시기 바랍니다.</p>
          </div>
        </div>
        <div className="mx-auto w-full max-w-sm shrink-0 space-y-4 rounded-md border p-4">
          <p className="text-lg font-semibold">최종 결제 정보</p>
          <div className="space-y-2">
            <div className="flex items-center justify-between">
              <p className="text-sm text-foreground/60">설명/구분</p>
              <p>
                {product.data.productCateCode === "C"
                  ? "패키지"
                  : productTypeCodeName(product.data.productTypeCode || "")}
              </p>
            </div>
            <div className="flex items-center justify-between">
              <p className="text-sm text-foreground/60">서비스기간</p>
              {product.data.productTypeCode === "TICKET" ? (
                <p>1회</p>
              ) : (
                <p>{`${new Date().toLocaleDateString()} ~ ${new Date(product.data.term || "").toLocaleDateString()}`}</p>
              )}
            </div>
            <div className="flex items-center justify-between">
              <p className="text-sm text-foreground/60">상품금액</p>
              <p>{formatPrice(Number(product.data.productPrice))}</p>
            </div>
            <div className="flex items-center justify-between">
              <p className="text-sm text-foreground/60">할인금액</p>
              <p>- {formatPrice(discountAmount)}</p>
            </div>
            <div className="flex items-center justify-between">
              <p className="text-sm text-foreground/60">최종결제금액</p>
              <p className="font-semibold text-primary">
                {formatPrice(
                  Number(product.data.productPrice) - discountAmount,
                )}
              </p>
            </div>
          </div>
          <Button className="w-full" onClick={requestPay}>
            결제하기
          </Button>
        </div>
      </div>
