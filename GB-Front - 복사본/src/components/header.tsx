...
export const Header = () => {
  ...
  // 선생님 여부 확인
  const isTeacher = user?.memberType === "teacher";
                  ...
                  <div className="w-full space-y-2">
                    ...
                    {user ? (
                      <>
                        ...
                        {isTeacher && <Link to="/mentor" className={cn(buttonVariants({ variant: "ghost" }), "w-full justify-start px-1")} onClick={() => setIsOpen(false)}>멘토 대시보드</Link>}
            ...
            {user ? (
              <Popover>
                ...
                <PopoverContent className="w-48 space-y-1 p-1">
                  ...
                  {isTeacher && <Link to="/mentor" className="flex h-8 w-full items-center rounded-md px-2 text-sm hover:bg-gray-100">멘토 대시보드</Link>}
