...
import { StyledCalendar } from '../pagecomp/styledcalendar'
import SvgIcon from '@material-ui/core/SvgIcon'
import axios from 'axios'
import Select from '../pagecomp/formselect'
...
const formData = (stuff) => stuff.reduce((acc,obj) => {
		const a = {...obj};
...
const EditLayout = React.forwardRef(({ onFieldChange, appointmentData, classes, children,errors, ...restProps },ref) => {
	...
	const days = {
	  ...
	  const onDescriptionChange = React.useCallback((nextValue) => {
		  console.log(nextValue)
		onFieldChange({ description: nextValue });
	  },[onFieldChange]);
	   const onProgressChange = React.useCallback((e, nextValue) => onFieldChange({progress : nextValue}),[onFieldChange])
	  ...
	  const onMentorDescChange = React.useCallback((nextValue) => {
	  ...
	  return (
		<div className={classes.root}>
			...
			<div className={classes.display}>
				 <div><p>과목</p><p>{appointmentData.subject}</p></div>
				 <div><p>{appointmentData.primaryType == '수업' ? '선생님명' : '학습 목표'}</p><p>{appointmentData.primaryType == '수업' ? appointmentData.teacher : appointmentData.title}</p></div>
				 <div><p>{appointmentData.primaryType == '수업' ? '수업목표' : '교재/인강'}</p><p>{appointmentData.primaryType == '수업' ? appointmentData.title : appointmentData.studyContent ? appointmentData.studyContent : '(없음)'}</p></div>
			</div>
			{appointmentData.primaryType == '수업' ? <>
				...
				<div>
					<Checkbox
					  value={appointmentData.late}
					  onClick={onLate}
					/>
					<span>지각</span>
					<Checkbox
					  value={appointmentData.absent}
					  onClick={onAbsent}
					/>
					<span>결석</span>
				</div>
...
const BasicLayout = React.forwardRef(({ onFieldChange, appointmentData, classes, children,errors, ...restProps },ref) => {
	...
	const days = {
	  ...
	  const onTitleChange = React.useCallback((nextValue) => {
	  ...
	  const changeDate = React.useCallback(date => {onFieldChange({ date: date , startDate: new Date(date.getFullYear(),date.getMonth(),date.getDate(),0,0,0), endDate: new Date(date.getFullYear(),date.getMonth(),date.getDate(),0,0,0)})}, [onFieldChange]);
	  ...
	  const onDayChange = React.useCallback(e => {
		  ...
		  if (!rRule) rRule = 'FREQ=WEEKLY;BYDAY='+days[v]+';INTERVAL=1' + (count && appointmentData.count ? ';COUNT='+appointmentData.count : '') + (!count && appointmentData.end ? ';UNTIL='+appointmentData.end.toISOString().split('T')[0].split('-').join('')+'T150000Z' : '')
		  else {
			rRule = rRule.split(';')[1]
			...
			else rRule = 'FREQ=WEEKLY;BYDAY='+str.slice(0,-1)+';INTERVAL=1' + (count && appointmentData.count ? ';COUNT='+appointmentData.count : '') + (!count && appointmentData.end ? ';UNTIL='+appointmentData.end.toISOString().split('T')[0].split('-').join('')+'T150000Z' : '')
		  }
		  onFieldChange({rRule: rRule})
	  },[onFieldChange,appointmentData,count])
	  const changeRecurrenceInterval = React.useCallback(v => {
		  let rRule = appointmentData.rRule
		  if (!rRule) return;
		  else {
			  rRule = rRule.split(';').slice(0,3).join(';')
			rRule = rRule + (count ? ';COUNT='+v : '') + (!count ? ';UNTIL='+v.toISOString().split('T')[0].split('-').join('')+'T150000Z' : '')
		  }
	  ...
	  return (
		<div className={classes.root}>
		  ...
		  <AppointmentForm.TextEditor
			...
			<FormControlLabel checked={repeat} onClick={() => setRepeat(!repeat)} control={<Radio />} label="반복주기 설정 (이 계획이 1일 이상 지속될 경우)" />
				{
					repeat ?
					<>
						<p>{appointmentData.rRule}
						</p>
						<AppointmentForm.Label
						text="주간 반복주기 선택"
						type="title"
						...
						<div className={classes.selector}>
							{
								['월','화','수','목','금','토','일'].map(e => <button value={e} id={e} onClick={onDayChange} className={appointmentData.rRule && appointmentData.rRule.split(';')[1].includes(days[e]) ? classes.active : ''}>{e}</button>)
							}
						</div>
						<div className={classes.menu}>
							<div>
								<FormControlLabel checked={count} onClick={() => setCount(!count)} control={<Radio />} label="플랜 수로 설정" />
								<AppointmentForm.TextEditor
								  readOnly={!count}
							...
							<div>
								<FormControlLabel checked={!count} onClick={() => setCount(!count)} control={<Radio />} label="끝나는 날짜로 설정" />
								<AppointmentForm.DateEditor
								  readOnly={count}
...
const IntegratedDates = ({ setStart, setEnd }) => {
  const s = (props) => {setStart(startViewDateComputed(props))}
  const e = (props) => {setEnd(endViewDateComputed(props))}
...
const Y = ({data,setData,date, setDate, setStart, setEnd}) => {
	...
	const Command = (props) => {
		return <AppointmentForm.CommandButton {...props} onExecute={(p)=>{
	...
	const Layout = (props) => props.appointmentData.id ? <EditLayout {...props} classes={selectClasses} ref={reffy} errors={errors}/> : <BasicLayout {...props} classes={selectClasses} ref={reffy} errors={errors}/>
      ...
      <Paper>
        ...
        >
		  ...
		  <EditRecurrenceMenu 
			...
			messages={{current:'이 계획만', currentAndFollowing:'이 계획과 따라오는 계획들', all:'모든 반복되는 계획들', menuEditingTitle:'반복되는 계획 변경하기', menuDeletingTitle:'반복되는 계획 삭제하기', cancelButton:'취소'}}
		  />
		  <AppointmentTooltip 
			showOpenButton
			showDeleteButton
...
const Monthly = ({date, data, start, end}) => {
	...
	const subjectsFiltered = React.useMemo(() => {
		...
		if (!start || !end) return [];
		else return data.reduce((acc,obj) => {
		if (obj.startDate.getTime() <= end.getTime() && start.getTime() < obj.endDate.getTime()) {acc.push(obj)}
		return acc},[])},[date,data,start,end])
	
	const def = (<>
	<style jsx>{`
	h3 {font-size:1.4rem;}
	.subject {padding:0.5em 1.2em; margin:1em 0; background-color:#f7f7f7; border-radius:10px; line-height: 1.8em; font-size:0.9rem; word-break:keep-all;}
				 .subject > a {margin-right:1em; color:#333333; opacity: 30%;}
				 .subject > a.on {opacity: 1;}
		.circle {display:inline-block; width:8px; height:8px; margin:0 4px 2px 0; border-radius:50%;}
				.kor {background-color:#f56262;}
				.math {background-color:#d8dc6a;}
				.eng {background-color:#fccf2e;}
				.his {background-color:#995d81;}
				.sci {background-color:#6689a1;}
				.etc {background-color:#f6afc0;}
	`}</style>
		<h3>이번주 계획</h3>
		<div className="subject">
			<a href="#" className={subject == '전체' ? 'on' : null} onClick={() => setSubject('전체')}>전체</a>
			<a href="#" className={subject == '국어' ? 'on' : null} onClick={() => setSubject('국어')}><span className="circle kor"></span>국어</a>
			<a href="#" className={subject == '수학' ? 'on' : null} onClick={() => setSubject('수학')}><span className="circle math"></span>수학</a>
			<a href="#" className={subject == '영어' ? 'on' : null} onClick={() => setSubject('영어')}><span className="circle eng"></span>영어</a>
			<a href="#" className={subject == '과학' ? 'on' : null} onClick={() => setSubject('과탐')}><span className="circle sci"></span>과탐</a>
			<a href="#" className={subject == '과학' ? 'on' : null} onClick={() => setSubject('사탐')}><span className="circle sci"></span>사탐</a>
			<a href="#" className={subject == '기타' ? 'on' : null} onClick={() => setSubject('기타')}><span className="circle etc"></span>기타</a>
		</div>
		<div className="detail">
		{
			subjectsFiltered.filter(e => subject == '전체' || subject == e.subject).map((d,i) => {return (<div className={s.flex} style={{alignItems:'center'}}>
				<SubjectListItem data={d} style={{width:0, flex:'1 0 0'}} /></div>)})
		}
		</div>
		</>)
/*onClick={() => {
	...
	return (
		<>
			<style jsx>{`
				 #con_right_top {min-height:700px; margin-bottom:2em; background-color:#ffffff; border-radius: 20px;}
				 #con_right_bottom {min-height:600px; background-color:#ffffff; border-radius: 20px;}
				 #con_right_bottom > h4 {font-size:1.4rem;}
...
const SubjectListItem = (props) => { //장기 리스트 뷰에서 나오는 목표 리스트 아이템들
	...
	return (
		<div style={{width:'100%', padding:'12px 20px', boxSizing:'border-box', ...props.style}} onClick={props.onClick ? () => props.onClick(data) : null}>
			<div className={[s.flex,s.smallfont].join(' ')} style={{marginBottom:'8px'}}>
				<div><div className={[s.dot,s[colors[data.subject]]].join(' ')}></div>{data.subject}&nbsp;|&nbsp;{data.primaryType}</div>
			</div>
			...
			{Math.ceil((endDate.getTime()-data.startDate.getTime())/(1000*60*60*24*7))}주</> : null }
			...
			<div style={{width:'100%',marginBottom:'8px'}} className={[s.flex,s.smallfont,s.disabled].join(' ')}>
				<div style={{borderRight:'1px solid black',paddingRight:'1em',overflow:'hidden'}} className={[s.ellipse,s.flexchild].join(' ')}>{data.primaryType == '수업' ? data.teacher : data.studyType && data.studyContent ? data.studyType+': '+data.studyContent : '(없음)'}</div><div className={s.flexchild} style={{paddingLeft:'1em'}}>{data.progress || '0'}%)</div>
			</div>
			<div style={{position:'relative'}}>
			<div className={[s.bar,s[colors[data.subject]]].join(' ')} style={{width: (data.progress || '0')+'%',position:'absolute'}} />
			<div className={[s.bar,s[colors[data.subject]],s.disabled].join(' ')}>
			</div>
			</div></>
			}
		</div>
...
const MobileBigCalendar = () => {
	...
	useEffect (() => {
		axios.get('/api/plan/add',{
			headers: {
				auth:  connected ? connected : localStorage.getItem('uid')
			}
		}).then(res => {
			const a = res.data.data.map(r => {r.startDate = new Date(r.startDate); r.endDate = new Date(r.endDate); return r})
			setData(a)
		})
	},[])
...
const DesktopBigCalendar = () => {
	...
	return (
		<DataContext.Provider value={{data: [data,setData], innerRef:ref, connected: connected}}>
			...
			<style jsx>{`
				/* reset */
				* {margin:0px; padding:0px; list-style:none; font-family:"Noto Sans CJK KR";}
				...
				#inner {display:flex; align-content:space-between; max-width:1216px; margin:6em auto 10em;}
				#inner .inner_top {height:47px; margin-bottom:1em;}
				#inner > #inner_left {width:66%; margin-right:2%;}
				#inner > #inner_left > .inner_top {display:flex; justify-content: space-between; align-items: center;}
				#inner > #inner_left > .inner_top > h1 {font-size:1.8rem;}
				#inner > #inner_left > .inner_top > ul {display:flex; height:36px; line-height:34px;}
				#inner > #inner_left > .inner_top > ul > li {position:relative; padding:0 12px; margin-left:20px; border-radius: 35px; font-weight: 500;}
				#inner > #inner_left > .inner_top > ul > li:nth-child(1) {margin-left:0; background-color: #de6b3d; color:#ffffff;}
				#inner > #inner_left > .inner_top > ul > li:nth-child(1):after {position:absolute; top:51%; right:-14px; transform:translateY(-50%); content:''; width:8px; height:9px; background-image:url('../public/assets/icons/p_nav_on.png');}
				#inner > #inner_left > .inner_top > ul > li:nth-child(2) {background-color:#ffffff; color:#333333;}
				#inner > #inner_left > .inner_top > ul > li:nth-child(2):after {position:absolute; top:51%; right:-14px; transform:translateY(-50%); content:''; width:8px; height:9px; background-image:url('../public/assets/icons/p_nav_off.png');}
				#inner > #inner_left > .inner_top > ul > li:last-child {background-color:#ffffff; color:#333333;}
				#inner > #inner_left > #con_left {min-height:1000px; padding:2em; background-color:#ffffff; border-radius:20px;}
				#inner > #inner_left > #con_left > h2 {font-size:2rem;}
				#inner > #inner_left > #con_left > h2 > a {padding:0 0.5em; vertical-align: middle;}
				#inner > #inner_right {width:32%;}
				#inner > #inner_right > .inner_top {line-height: 47px; text-align: right;}
				#inner > #inner_right > .inner_top > a {font-weight:500;}
				#inner > #inner_right > .inner_top > a:last-child {margin-left:1.2em;}
				#inner > #inner_right > .inner_top > a:last-child img {vertical-align: middle;}
				#inner > #inner_right > #con_right {min-height:1000px; padding:1em; background-color:#ffffff; border-radius: 20px; position:relative;}
				#inner > #inner_right > #con_right > h3 {font-size:1.4rem;}
				...
				/* start */
				#inner > #inner_right > #con_right > .detail > .detail_box {padding:1em; margin-top:1em; box-shadow:2px 2px 5px 0px #dddddd96; border-radius:10px;}
				#inner > #inner_right > #con_right > .detail > .detail_box > .subject {display:flex; font-size:0.7rem;}
				#inner > #inner_right > #con_right > .detail > .detail_box > .subject > .tit {margin-right:9px;}
				#inner > #inner_right > #con_right > .detail > .detail_box > .subject > .tit:after {display:inline-block; width:1px; height:9px; margin-left:9px; content:''; background-color:#8c8c8c;}
				#inner > #inner_right > #con_right > .detail > .detail_box > .subject > .tit > .circle {width:7px; height:7px; margin-bottom:1px;}
				#inner > #inner_right > #con_right > .detail > .detail_box > .title {padding:0.3em 0; font-size:1.1rem; font-weight:500; white-space:nowrap; overflow:hidden; text-overflow:ellipsis;color:black;}
				#inner > #inner_right > #con_right > .detail > .detail_box > .date {font-size:0.8rem;}
				#inner > #inner_right > #con_right > .detail > .detail_box > .learn {display:flex; position: relative; justify-content:space-between; padding:0.7em 0; font-size:0.7rem; color:#999999;}
				#inner > #inner_right > #con_right > .detail > .detail_box > .learn > .book {display:block; white-space:nowrap; overflow:hidden; text-overflow:ellipsis; width:60%;}
				#inner > #inner_right > #con_right > .detail > .detail_box > .learn > .book:after {position:absolute; top:50%; right:36%; transform:translateY(-50%); width:1px; height:9px; margin-left:9px; content:''; background-color:#cccccc;}
				#inner > #inner_right > #con_right > .detail > .detail_box > .learn > .per {width:30%;}
				#inner > #inner_right > #con_right > .detail > .detail_box > .bar {width:100%; height:8px;}
				.disabled {
					width: 100vw;
					height: 100%;
			...
			<div id="inner">
				<div id="inner_left">
					...
					<div id="con_left">
						<Y data={data} setData={setData} date={date} setDate={setDater} setEnd={setEnd} setStart={setStart}/>
					</div>
				</div>
				<div id="inner_right">
					{/*<div className="inner_top">
