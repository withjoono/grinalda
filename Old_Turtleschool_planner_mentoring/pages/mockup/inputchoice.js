		...
		<div className="page" style={{color:'#2d2d2d'}}>
			<div style={{padding:'20px'}}>
				...
				<div style={{fontSize:'1.2em'}} style={{margin:'1em 0'}} className={s.bold}>
					<span className={s.orange_txt}>{localStorage.getItem('name')}</span>님의<br/>
					모의고사 성적을 입력해 주세요
					<p style={{fontSize:'0.6em', color:'#d3d3d3',marginTop:'-1.5em', WebkitTextStroke:0}}><br/>*OMR카드로 입력시 유형별/범위별 세부 성적 분석이 가능합니다.</p>
				</div>
				<div className={s.select}>
					<Link href="/mockup/gradeinput"><button onClick={()=>{sessionStorage.setItem('type',examtype); sessionStorage.setItem('grade',grade >= 3 ? grade : name.includes('1학년') ? 1 : 2)}}><p>점수로</p><p>입력하기</p></button></Link>
					<Link href="/mockup/omrinput"><button onClick={()=>{sessionStorage.setItem('type',examtype); sessionStorage.setItem('grade',grade >= 3 ? grade : name.includes('1학년') ? 1 : 2)}}><p>OMR카드로</p><p>입력하기</p></button></Link>
				</div>
			</div>
		</div>
	);
