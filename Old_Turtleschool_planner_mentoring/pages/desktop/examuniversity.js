...
const Examuniversity = (props) => {
	...
	const getRangeTwo = (value) => {
		console.log(value,totalRange[1], totalRange[0])
	...
	return (<>
		<style jsx>{`
			...
			.bigbtn {
					width: 240px;
					height: 51px;
					background: transparent linear-gradient(135deg, var(---de6b3d_main) 0%, #D86132 100%) 0% 0% no-repeat padding-box;
					background: transparent linear-gradient(135deg, #DE6B3D 0%, #D86132 100%) 0% 0% no-repeat padding-box;
		...
		`}</style>
			...
			<div style={{width:'1280px',margin:'0 auto 60px'}}>
				<div className="menu">
					<button className={left ? "menu_active" : undefined} onClick={()=>setLeft(true)}>대학별 검색</button>
					<button className={left ? undefined : "menu_active"} onClick={()=>setLeft(false)}>학과별 검색</button>
				</div>
				...
				<div className='univTable'>
					<div>
						...
						{
							area.map(a => <button value={a.code} className={areaCode == a.code ? 'rest ace' : 'rest'} onClick={(e) => setAreaCode(e.target.value)}>{a.name}</button>)
						}
					</div>
					<div>
						<div className='first'>수리 선택</div>
						{
							line.map(a => <button value={a.code} className={lineCode == a.code ? 'rest ace' : 'rest'} onClick={(e) => setLineCode(e.target.value)}>{a.name}</button>)
						}
					</div>
					{
						!left ?
						<>
							<div>
								<input value={searchQuery} onChange={(e) => {setSearchQuery(e.target.value)}} placeholder='학과 입력 하세요' className='in' />
							</div>
							<div className='groups'>
								{
									departmentGroup.map(a => <button value={a.name} className={group == a.name ? 'orange_txt' : ''} onClick={(e) => setGroup(e.target.value)}>{a.name}</button>)
								}
							</div>
						</> : null
					}
							...
							<div className='desktop_box'>
								<div className='graph'>
									<div className='gridarea'>
										{
											data.map(d => <div className='grid'>{getRange(d[1],d[0])}</div>)
										}
									</div>
									<div className='bottom'>
										{
											data.map(d => (<div>{d[2]}</div>))
										}
									</div>
								</div>
							</div>
								...
								<table className='desktop_box_table'>
									...
									</tr>
									{
										data.map(r => (
											<tr>
												<td>{r[2]}</td>
												<td><button className='smm' value={r[3]} onClick={click}>학과 합격 예측</button></td>
											</tr>
										))
									}
								</table>
						...
						<div className='tt'>
							<div className='desktop_box'>
								<div className='graph'>
									<div className='gridarea'>
										{
											data.map(d => <div className='grid'>{getRangeTwo(d[3]-d[4])}</div>)
										}
									</div>
									<div className='bottom'>
										{
											data.map(d => (<div><p>{d[1]}</p><p>{d[2]}</p></div>))
										}
									</div>
								</div>
							</div>
									...
									</tr>
									{
										data.map(d => (
											<tr>
												...
												<td>{d[4]+'점/'+d[3]+'점'} - {d[4] >= d[3] ? '합격' : '불합격'}</td>
											</tr>
										))
									}
								</table>
						...
						<div className='tt' style={{marginTop:'60px'}}>
							<div className='desktop_box'>
								<div className='graph'>
									<div className='gridarea'>
										{
											univData.map(d => <div className='grid'>{getRangeTwo(d[2]-d[3])}</div>)
										}
									</div>
									<div className='bottom'>
										{
											univData.map(d => (<div>{d[0]}</div>))
										}
									</div>
								</div>
							</div>
