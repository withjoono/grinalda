...
const examinput = ({subjectarea, subjectcode, examcode}) => {
	...
	const sorty = (l) => {
		...
		return l.filter(e => e != undefined).sort((a,b) => {return zz[Math.floor(parseInt(a[1])/10)*10] - zz[Math.floor(parseInt(b[1])/10)*10]})
	}
	...
	const get = () => {
		...
		Object.keys(subjectcode).map(e => {
			...
			if (cod >= 10 && cod < 20) {if (cod==10) obj[10][0].push([e,subjectcode[e]]); if (cod>10) {obj[10][1].push([e,subjectcode[e]]); obj[10][2].push([e,subjectcode[e]])}}
			if (cod >= 20 && cod < 30) {if (cod==20) obj[20][0].push([e,subjectcode[e]]); if (cod>20) {if (cod < 25) {obj[20][1].push([e,subjectcode[e]])} obj[20][2].push([e,subjectcode[e]])}}
		})
	...
	const handleGrade = (e) => {
		...
		setGrade(value)
		setExamtype(examcode.filter(e => (e.grade == parseInt(value) || (e.grade == 3 && value == '4')) && e.year == year).sort((a,b)=>a.id-b.id))
	}
	const handleYear = (e) => {
		const {value} = e.target;
		setYear(value)
		setExamtype(examcode.filter(e => (e.grade == grade || (e.grade == 3 && grade == 4)) && e.year == parseInt(value)).sort((a,b)=>a.id-b.id))
	}
	...
	useEffect(() => {
		if (c > 1) {
			axios.post('/api/exams', {exams: ctx.exams[0], type: type}, {headers: {auth: localStorage.getItem('uid')}});
			setDisabled(true);
			setSubmitted(false);
		}
		setC(c+1);
	...
	return (
		...
		<Menu index={0} title='성적 입력' />
			<div style={{width:'1280px', margin: '0 auto'}}>
				...
				<button id='0' className={left ? 'menu_active' : undefined}>OMR 카드로 입력하기</button>
				<button id='1' className={!left ? 'menu_active' : undefined}>점수로 입력하기</button>
				</div>
				<div className='desktop_box selectbar'>
					...
					<select defaultValue="" onChange={handleYear}>
						...
						{[2019,2020,2021].map(e =>  {if (examcode.filter(k => k.id > 0 && (k.grade == parseInt(grade) || (k.grade == 3 && grade == '4')) && k.year == e).length) return <option value={e}>{e}</option>})}
					</select>
					...
					{left ? <>
					<select value={area} onChange={(e) => {setArea(e.target.value);  setD(Array(lengths[e.target.value]).fill()); setCode(null)}}>
					...
					<select value={code} onChange={(e) => {handleCode(e.target.value)}}>
					<option value="">세부과목 선택</option>
					{
						area && cc[area][grade-1] ? cc[area][grade-1].map(e => {
							if (e) return <option value={e[1]}>{e[0]}</option>
						}) : null
					}
					</select></>
				...
				{!left ? <> <Form disabled={!grade || !year || !type} subjectcode={subjectcode} information={ctx.exams[0] || []} setInformation={ctx.exams[1]} submitted={submitted} type={type} grade={grade} conv={conv}/>
<div style={bttn} onClick={()=>{disabled ? setDisabled(false) : setSubmitted(true)}}>{disabled ? '수정하기' : '완료하기'}</div></>:
				<> <OMR disabled={!grade || !year || !type || !area || !code} input={[d, setD]} answers={answers} multi={area == 70 ? [22,23,24,25,26,27,28,29,30] : []}/> <div style={bttn} onClick={handleSubmit}>{answers.length > 0 ? '수정하기' : '채점하기'}</div>
					{answers.length > 0 ? <Link href="/mockup/mygrade"><div style={bttn}>분석하기</div></Link> : null} </>
				}
			</div>
		</div>
	);
