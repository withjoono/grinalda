...
const Result = () => {
	<div className='mobile_page'>
			...
			<div style={{lineHeight:'30px',fontSize:'22px',marginBottom:'12px',borderBottom:'5px solid #DE6B3D'}}>
...
const EarlyInput = ({areaCodes}) => {
	...
	const a = useMemo(() => Array(8).fill(0).map(e => {const t=Math.random()*8+1; return [t,Math.min(9,t+Math.random()*3)]}),[])
	...
	return (<div className='page' style={{width:'90%',margin:'0 auto'}}>
			...
			<div>
				<select>
					...
					{
						areaCodes.map(r => <option value={r.code} key={r.code}>{r.name}</option>)
					}
				</select>
			</div>
			<button className='orangebigbtn'>진단하기</button>
...
export async function getStaticProps () {
	const {rows} = await pool.query(`select * from codes where "isUse" = true and "groupId" = 1`)
