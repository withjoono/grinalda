...
const EarlyInput = () => {
	...
	const chooseData = () => {
		const rows = data.filter(row => row.grade == grade+1 && row.semester == sem+1).map(row => 
			{const {subjectarea, subjectcode, unit, originscore, averagescore, standarddeviation, achievement, persons, rank, id} = row;
			return [subjectarea, subjectcode, unit, originscore, averagescore, standarddeviation, achievement, persons, rank, id]});
		console.log(rows)
			rows.push(['','','','','','','','',''])
		return rows;
	}
