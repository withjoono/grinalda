	...
	return (<div className='page' style={{width:'90%',margin:'0 auto'}}>
			...
			<table className='t'>
				...
				</tr>
				{
					names.map((n,i) => (
						<tr>
							{i == 0 ? <td rowSpan='3' className='s'>특별전형<br/>(정원내)</td> : i == 3 ? <td rowSpan='12' className='s'>특별전형<br/>(정원외)</td> : null}
							<td>{n}</td>
							<td><button onClick={handleOptions} value={i} className={options[i] ? 'a b' : 'b'}></button></td>
						</tr>
					))
				}
			</table>
