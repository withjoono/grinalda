import page from '../desktop/earlygraph'
		...
		<div style={{width:'90%', margin:'0 auto'}}>
			<div className='textbox'>
				<div className='textbox_first'>
					...
					{ show ? <><div><p><br></br>평가는 평가 항목별로 A+, A0, A-, B, C 5단계로 평가되며, 결과에 대한 주석까지 함께 제공됩니다.</p></div>
					<div><p><span className='orange_txt'><br></br>이용절차는</span> 전공별 선생님을 검색한후 선생님을 선택하고 아래 결제창에서 결제<br/> 후
					세부 내용 입력창을 통해 구체적인 사항을 입력한 후 제출하시면 됩니다.</p></div></> : null}
				</div>
			...
			<div className='selectbar'>
				<div>
					<select value={dae} onChange={changeBig}>
						...
						{
							big.map(r => <option value={r.comn_cd}>{r.comn_nm}</option>)
						}
					</select>
				</div>
				<div>
					<select value={jung} onChange={changeMed}>
						...
						{
							medium[dae].map(r => <option value={r.comn_cd}>{r.comn_nm}</option>)
						}
					</select>
				</div>
				<div>
					<select value={so} onChange={changeSo}>
						...
						{
							small[jung].map(r => <option value={r.comn_cd}>{r.comn_nm}</option>)
						}
					</select>
				</div>
			</div>
		 ...
		 <div className='textbox'>
			<div><p><span className='orange_txt'>비교과 분석 결제는 </span> 수시합격예측 페이지를 통해서 들어와야만 이용하실 수 있습니다.</p></div>
		</div>
		<div className='c'><span className='b'>수시합격예측 바로가기</span><img src='/assets/icons/arrow_left.svg' style={{transform: 'scale(-1,1)'}} /></div>
		</div>
	</>
	)
}
export default withPayment(withDesktop(page,EarlyInput),null,'수시');

export async function getStaticProps() {
	const {rows} = await pool.query(`select * from commoncode where comn_grp_cd = $1 or comn_grp_cd = $2 or comn_grp_cd = $3 order by comn_grp_cd asc`,['A00001','A00002','A00003'])
	...
	smallDivison[''] = []
	smallDivison['0204'] = []
	console.log(bigDivision, mediumDivison, smallDivison)
	return {props: {big: bigDivision, medium:mediumDivison,small:smallDivison}}
}