...
export default async (req, res) => {
    ...
    if (type == 'google') {
        const tokens = code == 'refresh_token' ? await getGoogleAccess(req.query.refresh_token) : await getGoogleRefresh(code); //req.query의 code가 'refresh_token' 이면 액세스 코드만 갱신하고, 아니면 리프레시, 액세스 다 가져옴
        ...
        if (tokens.status == 200) {
            ...
            res.json({ success: true, data: { ...tokens.data, id: p.sub, name: p.name } });
            res.statusCode = 200;
            res.end();
        } else {
