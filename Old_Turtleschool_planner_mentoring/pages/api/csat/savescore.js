...
/*
    ...
    console.log('stringfy', subjects.reduce(stringfySubjects, ''));

    axios
        .get('http://localhost:3000/api/pay/payment', {
            headers: {
...
*/
    ...
    if (req.method == 'POST') {
        //1.저장전에 삭제
        await pool.query(`delete from csatidscore where memberid = $1 and year = $2 `, [req.headers.auth, s_year]);
      ...
      select $1, '1',
         ...
         case when substring(subject_a, 1, 1) in ('6','7','1','2') and subject_a != '1G' and percentage is null
          ...
          where not exists (select * from csatidscore z
