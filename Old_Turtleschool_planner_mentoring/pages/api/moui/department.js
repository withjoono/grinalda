...
export default async (req, res) => {
    ...
    switch (req.method) {
        case 'GET':
			const a = await pool.query(`select u."name",u.id,m.학과,m.표점,m.조합 from universities u inner join "convertUniName" cun on u."name" = cun.변환 inner join mouidata m on m.대학 = cun.이투스 where u."areaCode" = $1
and (m.문이과 = $2 or coalesce ($2, '1') = '') and m.학과 like concat('%',$3::text,'%') and m.계열 = $4`,[area,lin,query,group])
