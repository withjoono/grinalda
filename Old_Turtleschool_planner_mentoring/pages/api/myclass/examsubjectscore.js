...
export default async (req, res) => {
    ...
    let str_dvsn = req.query.dvsn;  //과목
    let str_id = req.query.id == null ? rows[0].id : req.query.id;  //이름
    let str_gradecd = req.query.gradecd == null ? rows[0].gradecode : req.query.gradecd;  //학년
    let str_examcd = req.query.examcd;  //시험코드

    switch (req.method) {
        case 'GET':
            const { plnrid, dvsn, cls, gradecode, id, examcd} = { plnrid: rows[0].plnrid,  dvsn: str_dvsn, cls: rows[0].cls, gradecode: str_gradecd, id: str_id, examcd : str_examcd}
            ...
            let { rows } = await pool.query(`
                    with res as (
                        ...
                        and m.id = $5
                        --and cast(p."gradeCode" as float) = e.grade
                        --and p."gradeCode" = $1
                        and e."typeId" = z.id
                        --and e.grade = z.grade
                        ...
                        and ((coalesce($6, '1')='1' and 1=1) or (coalesce($6, '1')!='1' and z.id = $6))
                        union all --플래너정보
                        select '0' as gubun,
                            m2.id, 				--학생id
                            m2.user_name ,		--학생명
                        ...
                        and e2.grade = $1
                        and e2."typeId" = z2.id
                        --and e2.grade = case when $1 in ('P1','A1') then 1 when $1 = 'B1' then 2 when $1 = 'C1' then 3 else $1 end --z2.grade
                        ...
                        and ((coalesce($6, '1')='1' and 1=1) or (coalesce($6, '1')!='1' and z2.id = $6))
                    )
                    select gubun, id, case when $5 = id then user_name else substring(user_name, 1, 1) || '**' end
                    , plnrid , "gradeCode" , cls,
                    ...
                    order by gubun, id
                    ;
                `, [gradecode, cls, plnrid, dvsn, id, examcd]
            )
