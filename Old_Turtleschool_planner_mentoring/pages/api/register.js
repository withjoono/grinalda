...
export default async (req, res) => {
	...
	if (type == 'kakaotalk') {
		...
		console.log(result, confirmed, uid)
	} else if (type == 'google') {
	...
	let data = {}
	if (confirmed) {
		rows = await pool.query(`INSERT INTO members (account,cellphone,cert_path,email,"gradeCode","isPay",join_date,"payDate",push_token,"relationCode",school,"updateDate",user_name,imp_uid,merchant_uid)
										VALUES ($1,null,'kk',null,null,false,timezone('utc'::text, now()),null,null,null,null,timezone('utc'::text, now()),$2,'','')`,[uid,name])
