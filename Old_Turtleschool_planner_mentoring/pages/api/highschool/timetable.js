...
export default async (req, res) => {
	...
	if (req.method = 'GET') {
		...
		await axios.get('https://open.neis.go.kr/hub/hisTimetable',{
			params: {
				...
				ALL_TI_YMD:''+year+(month.toString().padStart(2,'0'))
			}
		}).then(response => {
			if (response.data['RESULT']) {
				...
				res.json({success: true, msg: 'success', data: response.data.hisTimetable});
				res.statusCode = 200;
			}	
			res.end();
		}
