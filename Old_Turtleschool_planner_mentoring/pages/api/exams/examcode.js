...
export default async (req, res) => {
    ...
    and     "isFrequent"    =   $3
		...
		if (req.body.answers) {
			...
			const whatever = await pool.query(`
				select * from conversion where "typeId" = $1 and "subjectCode" = $2 and "originScore" = $3
    ...
    select	$1 as "memberId"
		...
		on conflict ("memberId", "subjectArea", "subjectCode", "recruitYear", "isFrequent", "typeId")
            ...
            `, [memberId, recruitYear, isFrequent, req.body.area, req.body.code, originScore, type, req.body.answers,std,per,gra])
			if (req.body.area != 10 && req.body.area != 20){
				await pool.query(`delete from exams where "memberId"=$1 and "typeId"=$2 and "subjectArea"=$3 and "subjectCode"!=$4`,[memberId, type, req.body.area, req.body.code])
            } else {
				await pool.query(`delete from exams where "memberId" = $1 and "typeId" = $2 and ("subjectArea" = '10' or "subjectArea"='20') and id not in (select id from (select id from exams where "memberId" = $1 and "typeId" = $2 and ("subjectArea" = '10' or "subjectArea"='20') order by id desc limit 2) as x) `,[memberId, type])
			}
			success = true;
    ...
    from	(
			...
			if (e.subjectArea != 10 && e.subjectArea != 20){
				await pool.query(`delete from exams where "memberId"=$1 and "typeId"=$2 and "subjectArea"=$3 and "subjectCode"!=$4`,[memberId, type, e.subjectArea, e.subjectCode])
            } else {
				await pool.query(`delete from exams where "memberId" = $1 and "typeId" = $2 and ("subjectArea" = '10' or "subjectArea"='20') and id not in (select id from (select id from exams where "memberId" = $1 and "typeId" = $2 and ("subjectArea" = '10' or "subjectArea"='20') order by id desc limit 2) as x) `,[memberId, type])
			}
			})
