...
export default async (req, res) => {
	...
	const code = Array(6).fill().map(e=>{return random.charAt(Math.floor(Math.random() * random.length))}).join('')
	let data = await pool.query(`insert into tempcode values ($1, $2, now() + interval '5 minutes') returning expire`,[memberId, code]);
