...
export default async (req, res) => {
	...
	and		e."전형" = $4
			...
			rows.map(r => {
				const ans = {}
				if (r['1단계학생부'] || r['1단계면접'] || r['1단계실기'] || r['1단계서류'] || r['1단계수능'] || r['1단계기타'] || r['논술반영'] || r['논술학생부반영']) {
					ans['1단계'] = {
					...
					if (r['1단계배수']) {
						ans['1단계'].b = r['1단계배수']
						if (r['2단계학생부'] || r['2단계면접'] || r['2단계실기'] || r['2단계서류'] || r['2단계수능'] || r['2단계기타']) {
							ans['2단계'] = {
								h: r['2단계학생부'],
								m: r['2단계면접'],
								s: r['2단계실기'],
				...
				if (r['전교과'] != null) {
					...
					if (r['전교과']) {
						...
						['국어','수학','영어','사회','과학','통합사회','통합과학','한국사','제2외국어','한문'].map(w =>
						{
							if (r[w] != '  '){
				...
				if (r['수능영역수'] && r['수능영역수'] != ''){
					ans['수능반영'] = [r['활용지표'],r['수능영역수'],r['수능국어반영'],r['수능국어선택'],r['수능수학반영'],r['수능수학선택'],r['수능영어'],r['수능탐구반영'],r['수능탐구선택'],r['수능탐구반영수'],r['수능탐구지정과목'],
					r['수능외국어'],r['수능한국사']]
				}
				if (r['면접기준']){
				...
				if (r['최저수능영역수']) {
					ans['수능최저'] = [r['최저수능영역수'],r['최저수능국어반영'],r['최저수능국어선택'],r['최저수능수학반영'],r['최저수능수학선택'],r['최저수능영어'],r['최저수능탐구반영'],r['최저수능탐구선택'],
					r['최저수능외국어'],r['최저수능한국사'],r['최저세부내용']]
				}
				rr.push(ans)
			})
            data = rr;
