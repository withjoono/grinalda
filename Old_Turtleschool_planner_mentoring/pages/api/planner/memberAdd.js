...
export default async (req, res) => {
    let { rows } = await pool.query(`select m.id ,
    ...
    switch (req.method) {
        case 'POST':
            const { id, cls, gradecd, plnrid} = { id: rows[0].id, cls: str_cls, gradecd: rows[0].grade, plnrid: str_plnrid}
            //기존플랜너정보 사용중지
            await pool.query(`
            ...
            //신규플래너정보 사용시작
            await pool.query(`insert into plannermanagement
