...
export default async (req, res) => {
	...
	try {
	  ...
	  const {rows} = await pool.query(`select * from paymenttypes where name = $1`,[name])
	  const amountToBePaid = parseInt(rows[0].price);
	  if (amount === amountToBePaid) { // 결제 금액 일치. 결제 된 금액 === 결제 되어야 하는 금액
        switch (status) {
          case "paid": // 결제 완료
			const payrow = await pool.query(`update payments
											set confirmed = true
											where imp_uid = $1
											returning payments.*`,[imp_uid]);
				
			const {rows} = await pool.query(`select push_token from members where id = $1`,[payrow.rows[0].accountid]);
			if (rows[0].push_token !== '') {
				const {vbank_name, vbank_num, vbank_holder} = paymentData;
				await axios.post('https://ingipsy.com/api/notification', {push_token: rows[0].push_token, title: "결제 완료", message:`결제 타입:${name}\n금액:${amount}\n${vbank_name}\n${vbank_num}\n${vbank_holder}\n`});
			}
