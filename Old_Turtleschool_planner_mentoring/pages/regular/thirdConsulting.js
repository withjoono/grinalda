...
const ThirdConsulting = () => {
    ...
    const renderContentComponent = () => {
        ...
        return <Content type="다" selectedUniv={selectedUniv} onMajorClick={onMajorClick} onMajorsClick={onMajorsClick}/>;
    };
    ...
    return (
        <UserAgentProvider ua={window.navigator.userAgent}>
            <UserAgent computer>
                <SideNavPage
                    ...
                    <NavButtonContainer>
                        ...
                        <NavButton disabled={page === 4} active={page !== 4} onClick={() => setPage((prev) => prev + 1)}>
                            {'다음 단계'}
                        </NavButton>
                    </NavButtonContainer>
                </SideNavPage>
            ...
            <UserAgent mobile>
                <SideNavPage routes={['홈', '정시 합격 예측', '다군 컨설팅']} navTitle='정시 합격 예측 - 다군 컨설팅' currentPage={pageTitles[page].name} navSubs={pageTitles.map((title) => ({ title: title.name, url: '#' }))}>
                    {renderContentComponent()}
                    <NavButtonContainer>
                        <NavButton disabled={page === 0} active={page !== 0} onClick={() => setPage((prev) => prev - 1)}>
                            {'이전 단계'}
