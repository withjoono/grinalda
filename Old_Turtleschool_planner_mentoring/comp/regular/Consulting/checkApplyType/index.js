...
const outTypes = ['농어촌 학생', '특성화고교 졸업자', '특성화고 등을 졸업한 재직자', '기초생활 수급자, 차상위 계층, 한부모가족 지원대상자', '장애인 등 대상자', '계약학과', '재외국인 및 외국인'];

const CheckApplyType = () => {
    ...
    const renderCheckBox = (title) => (
        <S.CheckBoxContent key={title} active={selectedType === title}>
            <S.CheckBoxInput type='checkbox' id={title} checked={selectedType === title} onChange={onCheckBoxClick} />
            <label htmlFor={title} />
    ...
    return (
        <S.Container>
            ...
            <S.Section>
                ...
                <S.CheckBoxContainer>{outTypes.map(renderCheckBox)}</S.CheckBoxContainer>
            </S.Section>
            <S.SaveButtonLayout>
                <Button
