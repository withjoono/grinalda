...
const ScoreInput = ({ selectedUnivCode, addUnivCodes, deleteUnivCodes, userGrade, setUserGrade, userScore, setUserScore }) => {
    ...
    const renderScoreInput = (key) => <td key={key}>{key === 'eng' ? '-' : <input name={key} type='number' min={0} max={100} onBlur={fillInput} onFocus={clearInput} value={userScore[key]} onChange={onScoreChange} placeholder='점수' className={styles.b_gray_05} />}</td>;
