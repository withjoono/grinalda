...
const MajorSelection = ({ selectedUnivCode, secondSciUnivCode, gpaUnivCode, finalUnivCode, addUnivCodes, deleteUnivCodes }) => {
    ...
    const checkSuitability = (univ) => {
        if (univ.universityid === '355' || univ.universityid === '241') {
            const isRequired = [univ.scncpa, univ.scncca, univ.scncba, univ.scncea, univ.scncpb, univ.scnccb, univ.scncbb, univ.scnceb].map((value) => (value === '○' ? true : false));
            // 하나라도 포함되면 부합
            const result = Object.values(filter).filter((value, index) => value && value === isRequired[index]).length;
            return result;
        } else {
            return true;
        }
    ...
    const renderUnivItem = (univ, index) => {
        ...
        return (
            <tr key={univ.universityid + ',' + univ.departmentid + ',' + index} className={cx('t_br', 't_bb', 't_bl')}>
        ...
        <div id={cx('contents')}>
            <div id={cx('discuss')}>
                <div id={cx('discuss_contents')}>
                    <Advice>
                        * 연대와 건국대는 학과마다 과학 선택 과목이 다릅니다. <br></br>* 학과별 과학 지정 여부를 다시 확인해주세요.
                    </Advice>
                                    ...
                                    <div className={otherCx('discuss_row1')}>
                                        <table className={otherCx('discuss_table1')}>
                                            ...
                                            <tbody>
                                                ...
                                                <tr>
                                                    <td>
                                                        <button name={Object.keys(filter)[0]} onClick={onFilterClick} className={otherCx('check', filter.physics1 ? 'c_orange' : ['c_gray_02', 'b_gray_06'])}>
                                                            물1
                                                        </button>
                                                        <button name={Object.keys(filter)[1]} onClick={onFilterClick} className={otherCx('check', filter.chemistry1 ? 'c_orange' : ['c_gray_02', 'b_gray_06'])}>
                                                            화1
                                                        </button>
                                                        <button name={Object.keys(filter)[2]} onClick={onFilterClick} className={otherCx('check', filter.biology1 ? 'c_orange' : ['c_gray_02', 'b_gray_06'])}>
                                                            생1
                                                        </button>
                                                        <button name={Object.keys(filter)[3]} onClick={onFilterClick} className={otherCx('check', filter.earth1 ? 'c_orange' : ['c_gray_02', 'b_gray_06'])}>
                                                            지1
                                                        </button>
                                                    </td>
                                                </tr>
                                                <tr className={otherCx('t_bb')}>
                                                    <td>
                                                        <button name={Object.keys(filter)[4]} onClick={onFilterClick} className={otherCx('check', filter.physics2 ? 'c_orange' : ['c_gray_02', 'b_gray_06'])}>
                                                            물2
                                                        </button>
                                                        <button name={Object.keys(filter)[5]} onClick={onFilterClick} className={otherCx('check', filter.chemistry2 ? 'c_orange' : ['c_gray_02', 'b_gray_06'])}>
                                                            화2
                                                        </button>
                                                        <button name={Object.keys(filter)[6]} onClick={onFilterClick} className={otherCx('check', filter.biology2 ? 'c_orange' : ['c_gray_02', 'b_gray_06'])}>
                                                            생2
                                                        </button>
                                                        <button name={Object.keys(filter)[7]} onClick={onFilterClick} className={otherCx('check', filter.earth2 ? 'c_orange' : ['c_gray_02', 'b_gray_06'])}>
                                                            지2
                                                        </button>
                                                    </td>
                                                </tr>
