...
const examinput = ({subjectarea, subjectcode, examcode}) => {
  ...
  useEffect(() => {
    ...
    if (grade === 3 && year === 2022) {
      setExamtype(
        examcode
          .filter(
            e =>
              (e.grade == grade || (e.grade == 3 && grade == 4)) && e.year == 2022 && e.id === 187,
          )
          .sort((a, b) => a.id - b.id),
      );
  ...
  return (
    ...
    >
      <div className="page">
        ...
        `}</style>
        <Menu index={0} title="유리한 조건 찾기" />
        <div style={{width: '100%', margin: '0 auto'}}>
          <div className="menu" onClick={clicky}>
            <button id="0" className={left ? 'menu_active' : undefined}>
              OMR 카드로 입력하기
            </button>
            <button id="1" className={!left ? 'menu_active' : undefined}>
              점수로 입력하기
            </button>
          </div>
          <div className="desktop_box selectbar">
            <select defaultValue="" onChange={handleGrade}>
              {/* // 3학년 2022 6월 모의고사 고정을 위해 임시 주석 */}
            ...
            >
              ...
              {examtype.map((e, i) => {
                // 3학년 2022 6월 모의고사 고정을 위해 임시 함수 작성
                if (e.id === 187 && e.year === 2022 && e.grade === 3) {
                  return (
          ...
          {!left ? (
            ...
            <>
              ...
              {answers.length > 0 ? (
                <Link href="/mockup/mygrade" passHref>
                  <div style={bttn}>분석하기</div>
                </Link>
              ) : null}{' '}
            </>
          )}
          <Buttons prevPage="" nextPage="/early/option" />
        </div>
      </div>
    </SideNavPage>
  );
