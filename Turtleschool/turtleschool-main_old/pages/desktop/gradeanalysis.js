...
const Analysis = () => {
  ...
  useEffect(() => {
    axios
      ...
      .then(res => {
        chartone(res.data.data);
        averageGpa(res.data.data);
      });
  }, []);
  ...
  const averageGpa = e => {
    ...
    resulta = e.reduce((resulta, item) => resulta + item.averagegradea * 1 + 0.0, 0.0);
    resultb = e.reduce((resultb, item) => resultb + item.averagegradeb * 1 + 0.0, 0.0);
    resultc = e.reduce((resultc, item) => resultc + item.averagegradec * 1 + 0.0, 0.0);
    resultd = e.reduce((resultd, item) => resultd + item.averagegraded * 1 + 0.0, 0.0);
    resulte = e.reduce((resulte, item) => resulte + item.averagegradee * 1 + 0.0, 0.0);
    resultf = e.reduce((resultf, item) => resultf + item.averagegradef * 1 + 0.0, 0.0);
    resultg = e.reduce((resultg, item) => resultg + item.averagegradeg * 1 + 0.0, 0.0);

    const arr_data = [
      {
        averagegradea: Math.ceil((resulta / e.length) * 10) / 10,
  ...
  const Selectchartthree = e => {
    axios
      .get('/api/gpa/GpaChangesAnalysis', {
        headers: {
          auth: localStorage.getItem('uid'),
        },
        params: {
  ...
  function chartone(param) {
    const arr_names = param.map(obj => {
      return obj.grade + '학년' + obj.semester + '학기';
    });

    const arr_score = param.map(obj => {
      return obj.averagegradea * 1;
    });
