...
const gradeinput = () => {
  ...
  const changeMyData = e => {
    setData(p => {
      p[Math.floor(e.target.name / 12)][e.target.name % 12] = e.target.value;
      return [...p];
    }); //e 타겟 벨류 이중배열 쪽
  };
  ...
  const processData = d => {
    setRawData(d);
    const one = d
      .filter(row => row.semester == 1 && row.grade == grade)
      .map(row => {
        const converted = Object.keys(row).reduce((acc, key) => {
    ...
    const two = d
      .filter(row => row.semester == 2 && row.grade == grade)
      .map(row => {
        const converted = Object.keys(row).reduce((acc, key) => {
          acc[key] = row[key] == null ? '' : row[key];
          return acc;
  ...
  useEffect(() => {
    ...
    getData(
      '/api/codes/[groupName]',
      setcodeData13,
      {groupName: 'curriculum_Code'},
      localStorage.getItem('realuid'),
    );
    getData(
      '/api/codes/[groupName]',
      setcodeData14,
      {groupName: 'subject_Code'},
      localStorage.getItem('realuid'),
    );
  ...
  function openTextFile() {
    ...
    input.type = 'file';
    input.accept =
      'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet, application/vnd.ms-excel'; // 확장자가 xxx, yyy 일때, ".xxx, .yyy"
    ...
    reader.onload = async function () {
      ...
      await workBook.SheetNames.forEach(async function (sheetName) {
        ...
        if (i == 0) {
          ...
          if (
            Object.keys(user[0]).includes('학년') &&
            Object.keys(user[0]).includes('학기') &&
            Object.keys(user[0]).includes('교과')
          ) {
            await axios.post(
              '/api/gpa/Savegpa',
              {
            ...
            const queries = async item => {
              ...
              for (let i = 0; i < codeData13.length; i++) {
                if (
                  codeData13[i].description == item['교과'] ||
                  codeData13[i].name == item['교과']
                ) {
              ...
              for (let ib = 0; ib < codeData14.length; ib++) {
                if (
                  codeData14[ib].description == item['과목'] ||
                  codeData14[ib].name == item['과목']
                ) {
          ...
          //else if (Object.keys(user[0]).length == 17)
            ...
            Object.keys(user[0]).includes('2학기\r\n이수자수') &&
            Object.keys(user[0]).includes('1학기\r\n단위수') &&
            Object.keys(user[0]).includes('2학기\r\n단위수')
          ) {
            await axios.post(
      ...
      <div className="contain">
          ...
          <div className="buttonBorder">
            <div className="step1">
              <ul>
                ...
                <div
                  className={company == 'jinhak' ? 'companyChosen' : ''}
                  onClick={() => setCompany('jinhak')}
                >
                  <img src="https://img.ingipsy.com/assets/jinhak.png" />
                  진학사
                </div>
                <div
                  className={company != 'jinhak' ? 'companyChosen' : ''}
                  onClick={() => setCompany('uway')}
                >
                  <img src="https://img.ingipsy.com/assets/uway.png" />
                  유웨이
                </div>
              </ul>
            </div>
            <div className="step3">
              <ul>
                ...
                <a href="https://img.ingipsy.com/excel/거북스쿨 성적입력폼.xlsx" download>
                  <button>양식다운로드</button>
                </a>
              </ul>
              <p style={{fontSize: '16px'}}>
                {' '}
                <p>[거북스쿨 내신 샘플]로 입력하시면 학생부</p>
                <p>교과 · 학생부 종합 전형에 최적화된 분석 결과를 받으실 수 있습니다.</p>
              </p>
            </div>
            <div className="step2">
              <ul>
        ...
        <div className="notice">
          ...
          <div className="noticeBorder1">
            <div className="text">
              현재 연세대, 서울시립대 등 일부 상위권 대학은 수시의 모든 전형을{' '}
              <span className="color">Z점수</span>로 반영하고 있고,<br></br>
              학생부 종합의 경우 더 많은 대학들이 Z점수와 내신을 적절히 조합하여 학생 선발에
              활용하고 있습니다.<br></br>
              따라서 학생의 Z점수 분석은 필수이며, 이 점수를 파악하기 위해서는
              <br></br>
              <span className="color">
                원점수, 평균, 편차, 성취도 등 내신 입력 창의 칸을 모두 채우셔야 합니다.
              </span>
              <br></br>
              진학사 혹은 유웨이에 해당 정보를 입력하셨다면, 그곳에서 파일을 다운로드하신 후 여기에
              업로드하시면 됩니다.
            </div>
          </div>
          <div className="noticeBorder2">
            ...
            <ul>
              ...
              <li>
                5.소수점 첫째 자리 이상 입력되어 있는 경우 (평균점수/표준편차를 제외하고는 정수,
                평균점수/표준편차는 소수점 첫째 자리까지만 입력)
              </li>
              <li>
                6. 동일한 과목을 두 줄에 입력한 경우 (ex. 국어 1-2학기 이수 - 한 줄에 1학기, 한 줄에
                2학기 각각 입력)
              </li>
  ...
  return (
    <>
      <SideNavPage
        ...
        navTitle="교과/비교과 분석"
        navSubs={[
          {title: '1.성적입력', url: '/gpa/infoform'},
          {title: '2.교과 분석', url: '/gpa/mygrade'},
          {title: '3.비교과 분석', url: '/gpa/graph'},
          {title: '4.전형별 예측 대학', url: ''},
          {title: '5.목표대학', url: ''},
        ]}
      >
        <Menu index={0} />
        <div className="page" style={{marginBottom: '85px'}}>
          <div style={{margin: '0 auto'}}>
            <div className="menu" onClick={changeType}>
              <button value={0} className={type == 0 ? 'menu_active' : ''}>
                성적 직접 입력
              </button>
              <button
                value={1}
                className={type == 1 ? 'menu_active' : ''}
                onClick={() => setType(0)}
              >
                엑셀 파일 업로드
              </button>
            ...
            {type == 0 ? (
              <>
                ...
                >
                  <button value={1} className={grade == 1 ? 'menu_active' : ''}>
                    1학년
                  </button>
                  <button value={2} className={grade == 2 ? 'menu_active' : ''}>
                    2학년
                  </button>
                  <button value={3} className={grade == 3 ? 'menu_active' : ''}>
                    3학년
                  </button>
                </div>
                <Form
                  ...
                  semester={1}
                  data={data}
                  setData={changeMyData}
                  addData={addData}
                  submit={submitData}
                  subjectArea={codeData13}
                  subjectCode={codeData14}
