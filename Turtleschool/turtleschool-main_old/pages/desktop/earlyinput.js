      ...
      await workBook.SheetNames.forEach(async function (sheetName) {
        if (i == 0) {
          ...
          if (
            ...
            Object.keys(user[0]).includes('과목') &&
            Object.keys(user[0]).includes('단위수') &&
            Object.keys(user[0]).includes('수강자수')
          ) {
            await axios.post(
              '/api/gpa/Savegpa',
              {
      ...
      <div className="contain">
        ...
        <div className="notice">
          ...
          <div className="noticeBorder1">
            <div className="text">
              <span>수시의 모든 전형을</span>
              <span className="color">z점수</span>
              <span>
                로 내신 반영하는 대학이 연세대, 고대, 시립대 등 밖에 없지만, 학생부종합 전형에서는
                많은 대학들이 z점수와 내신을 적절하게 조합해서 사용
              </span>
              <div>하고 있습니다 그렇기 때문에,학생의 z점수 분석은 필수이며,</div>
              <span>
                이 z 점수를 파악하기 위해서는 내신 입력시 단위수와 평점만 입력하는 것이 아니라,
                원점수,평균, 편차,
              </span>
              <div className="color"> 성취도 등 아래 입력난을 모두 입력하셔야 합니다.</div>
              <div>
                진학사든 유웨이든 어느 한곳에 이런 모든것을 입력하셨다면, 그것을 받아서 입력하시면
                되며, 반대로 거북스쿨에서 입력하시면 다운 받아서 다른곳에 입력도 가능합
              </div>
              <div>니다.</div>
            </div>
          </div>
        ...
        <div className="page" style={{marginBottom: '85px'}}>
          <div style={{width: '100%', margin: '0 auto'}}>
            <div className="menu" onClick={changeType}>
              ...
              <button value={1} className={type == 1 ? 'menu_active' : ''}>
                엑셀 파일 업로드
              </button>
            </div>
            {type == 0 ? (
              <>
                ...
                />
                <Buttons prevPage={''} nextPage={'/early/analysis'} />
              </>
            ) : (
              excel
            )}
