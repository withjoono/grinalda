...
const Status = ({}) => {
  ...
  return (
    <SideNavPage
      routes={['홈', '플래너&멘토링', '학습현황/수업현황']}
      navTitle="플래너/멘토링"
      navSubs={[
        {title: '장기 학습/수업 계획', url: '/planner/plan'},
        {title: '금일 성취현황 및 멘토링', url: '/planner/result'},
        {
          title: '학습/수업 누적현황',
          url: '/planner/status',
        },
    ...
    >
      <TabContainer>
        <TabContent active={selectedTab === '학습현황'} id="학습현황" onClick={onTabClick}>
          {'학습현황'}
        </TabContent>
        <TabContent active={selectedTab === '수업현황'} id="수업현황" onClick={onTabClick}>
          {'수업현황'}
        </TabContent>
      </TabContainer>
      {renderComponent()}
