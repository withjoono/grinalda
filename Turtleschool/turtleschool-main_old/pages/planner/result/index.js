...
const EditLayout = React.forwardRef(
  (
    {onFieldChange, appointmentData, classes, children, errors, editingTarget, ...restProps},
    ref,
  ) => {
    useEffect(() => {
    ...
    const [test, setTest] = useState(
      appointmentData.score || appointmentData.rank || editingTarget?.score || editingTarget?.rank,
    );
    const [option, setOption] = useState(0);
    const [progress, setProgress] = useState(
      appointmentData.progress || editingTarget?.progress || 0,
    );
...
const SecondEditLayout = React.forwardRef(
  (
    {onFieldChange, appointmentData, classes, children, errors, editingTarget, ...restProps},
    ref,
  ) => {
      ...
      e => {
        ...
        if (!rRule)
          rRule =
            ...
            (!count && appointmentData.endDate
              ? ';UNTIL=' + getRruleDate(appointmentData.endDate)
              : '');
        else {
          rRule = rRule.split(';')[1];
    ...
    const changeRecurrenceInterval = React.useCallback(
      v => {
        ...
        if (!rRule) return;
          ...
          rRule =
            rRule + (count ? ';COUNT=' + v : '') + (!count ? ';UNTIL=' + getRruleDate(v) : '');
        }
        appointmentData = Object.assign(appointmentData, {rRule: rRule});
        onFieldChange({rRule: rRule});
      },
    ...
    return (
      <div className={classes.root}>
        ...
        <div className={classes.menu}>
          <AppointmentForm.Select
            value={editingTarget ? editingTarget.primaryType : appointmentData.primaryType}
            onValueChange={onPrimaryTypeChange}
            availableOptions={primaryTypeOptions}
            className={classes.select}
          />
          <AppointmentForm.Select
            value={editingTarget ? editingTarget.subject : appointmentData.subject}
        ...
        <AppointmentForm.Label text="계획명" type="title" />
        <AppointmentForm.TextEditor
          placeholder="계획 목표 또는 이름을 작성해 주세요"
          type="ordinaryTextEditor"
  ...
  ({onFieldChange, appointmentData, classes, children, errors, ...restProps}, ref) => {
    ...
    useEffect(() => {
      ...
      const result = Object.assign(newObj, {
        ...
        rRule: 'FREQ=WEEKLY;BYDAY=SU,MO,TU,WE,TH,FR,SA;INTERVAL=1;UNTIL=' + getRruleDate(end),
      });
    ...
    const changeStartDate = React.useCallback(
      startDate => onFieldChange({startDate, date: startDate}),
      [onFieldChange],
    );
    const changeEndDate = React.useCallback(
      endDate => onFieldChange({endDate, end: endDate}),
      [onFieldChange],
    );
    const changeDate = React.useCallback(
      date => {
    ...
    const onDayChange = e => {
      ...
      if (!rRule)
        rRule =
          ...
          ';INTERVAL=1' +
          (count && appointmentData.count ? ';COUNT=' + appointmentData.count : '') +
          (!count && appointmentData.end ? ';UNTIL=' + getRruleDate(appointmentData.endDate) : '');
      else {
        rRule = rRule.split(';')[1];
        const arr = Object.keys(days).map(key => rRule.includes(days[key]) ^ (key == v));
          ...
          <>
            ...
            {appointmentData.studyType ? (
              appointmentData.studyType === '인강' ? (
                <>
                  <input
                    onChange={onStudyContentChange}
                    style={{
                      width: '100%',
                      border: '1px solid black',
                      padding: 4,
                    }}
                    placeholder="인강 이름을 입력하세요."
                    list="lecture_list"
                    type="text"
                  />
...
const Y = ({
  ...
  const Command = props => {
    //세이브 버튼을 누를 시 정보가 제대로 입력되었는지 확인하는 기능 추가
    return (
      <AppointmentForm.CommandButton
        ...
        onExecute={p => {
          ...
          if (Object.keys(obj).length > 0) {
            ...
            props.onExecute(p);
          } //onExecute는 원래 세이브 버튼이 눌려질시 실행하는 함수
        }}
      />
    );
  };
      ...
      const res = await axios.post(
        '/api/plan/add',
        {
          ...
          allDay: false,
            ...
            added.startDate.getFullYear(),
            added.startDate.getMonth(),
            added.startDate.getDate(),
            added.endDate.getHours(),
            added.endDate.getMinutes(),
          ),
          endDate: new Date(
            added.startDate.getFullYear(),
            added.startDate.getMonth(),
  ...
  const UnclickableMonthTableCell = props => (
    <MonthView.TimeTableCell {...props} onDoubleClick={undefined} />
  );

  const UnclickableDayTableCell = props => (
        ...
        <Scheduler data={data} locale="ko-kr" height={800}>
          ...
          <Plugin dependencies={pluginDependencies} name="IntegratedDates">
            ...
            <EditRecurrenceMenu
              ...
              messages={{
                current: '오늘 하루만',
                currentAndFollowing: '오늘부터 남은 기간',
                all: '전체 기간',
                menuEditingTitle: '장기계획 변경하기',
                menuDeletingTitle: '장기계획 삭제하기',
                cancelButton: '취소',
              }}
            />
            ...
            {/*overlayComponent는 폼이 어느 div에서 나오는지 결정하는 컴포넌트
				commandButtonComponent는 파란 세이브 버튼 컴포넌트
...
const Z = ({
  ...
  const Layout = props =>
    props.appointmentData.id || editingTarget ? (
      type == 0 ? (
        <EditLayout
          {...props}
          classes={selectClasses}
    ...
    <>
      ...
      >
        ...
        {onDiscard ? (
          <button
            onClick={onDiscard}
            style={{padding: '6px 12px', fontWeight: 'bold', borderRadius: 4}}
          >
            {' '}
            닫기{' '}
          </button>
...
const SubjectListItem = props => {
  ...
  return (
    ...
    >
      <div
        className={[s.flex, s.smallfont].join(' ')}
        style={{
          marginBottom: '8px',
          display: 'flex',
          flexDirection: 'row',
      ...
      <div style={{position: 'absolute', overflow: 'hidden', width: '100%'}}>
        ...
        >
          <Monthly
            ...
            setFormVisible={setFormVisible}
            setEditingTarget={setEditingTarget}
            onDiscard={() => setListVisible(false)}
          />
        </div>
        {!formVisible && (
          <div
    ...
    <DataContext.Provider value={{data: [data, setData], innerRef: ref, connected: connected}}>
      <div style={{position: 'relative'}}>
        ...
        <div id="inner">
          ...
          <div id="inner_right">
            {/*<div className="inner_top">
						<a href="#"><img src="assets/icons/setting.png" alt="플래너 설정" /> 플래너 설정</a>
			</div>*/}
