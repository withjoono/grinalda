...
const MyGrade = ({personalcode, subjectcode, area, line, recruit_group, examcode}) => {
  ...
  useEffect(() => {
    postData('/api/sores/combine', setCombineResult, {
      combine: 10,
      type: type[0] ? type[0] : 0,
    });
  }, [combine]);
  ...
  return (
      ...
      <div className="page" style={{color: '#2d2d2d'}}>
        ...
        {/* style={{margin: '1em 0'}} */}
          <div style={{fontSize: '1.2em'}}  className={s.bold}>
            <span className={s.orange_txt}>{localStorage.getItem('name')}</span>
            님의
            <br />
            모의고사 정보을 선택해 주세요
          </div>
          <div className={s.gradebuttons}>
            ...
            <button
              className={grade == 4 ? s.active : null}
              onClick={() => {
                setGrade(4);
              }}
            >
          ...
          <button className="orangebigbtn" onClick={getStuff}>
            분석하기
          </button>
        </div>
        <div style={{fontSize: '1.2em', width: '90%', marginBottom: '1em'}}>
          <span style={{fontWeight: 'bold', color: '#fede01'}}>{localStorage.getItem('name')}</span>
          님의
        ...
        <div className={styles.MyGrade_buttons}>
          ...
          {gData.map((el, i) => {
            if (i == index)
              return (
                <button style={{'background-color': '#fede01', color: 'white'}}>
                  {el.subjectCode == 10 ? '통합사회' : el.subjectName}
                </button>
              );
            else
              return (
          ...
          {exams[0] ? (
            <>
              ...
              <div
                ...
                className={styles.MyGrade_buttons}
              >
                {exams[0].map(e => (
                  <button
                    className={subject.subjectCode == e.subjectCode ? 'active' : undefined}
                    key={e.subjectCode}
                    onClick={() => {
                      setSubject(e);
                      handleWow(e);
                  ...
                  >
                    {e.subjectCode == '20' && grade == 1 ? '과학탐구' : subjectcode[e.subjectCode]}
                  </button>
                ))}
              </div>
            </>
            ...
            <>
              ...
              {Object.keys(info.scores).map(e => (
                <div className="spacing outline equal">
                  ...
                  <div>
                    {Math.round(
                      (info.scores[e].right / (info.scores[e].right + info.scores[e].wrong)) * 100,
                    )}
                    %
                  </div>
                </div>
...
export async function getStaticProps() {
  ...
  const personalcode = rows.reduce((obj, entry) => {
    obj[entry.name] = entry.code;
    return obj;
  }, {});
  let dat;
  ...
  const sc = dat.rows.reduce((obj, entry) => {
    obj[entry.subjectArea] = entry.areaName;
    entry.child.map(v => {
      obj[v.subjectCode] = v.codeName;
    });
    return obj;
  }, {});
  dat = await pool.query(`
