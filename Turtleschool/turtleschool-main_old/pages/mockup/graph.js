...
const Chart = ({data, labels, title, subtitle, oneToOne, chosen}) => {
  ...
  useEffect(() => {
    ...
    setOptions({
      ...
      series: {
        data: data,
        name: ['', '표준점수', '전국백분위', '등급'][chosen],
        yAxis: parseInt(chosen - 1),
      },
      ...
      Object.keys(a)
        ...
        .map(key => {
          ...
          a[key].map(e => {
            ...
            sum3 = sum3.map(
              (k, i) =>
                k + (parseInt([e.originScore, e.standardScore, e.percentScore, e.grade][i]) || 0),
            );
            if (e.subjectArea != '80') {
              sum2 = sum2.map(
                (k, i) =>
              ...
              if (e.subjectArea != '50') {
                sum = sum.map(
                  (k, i) =>
                    k +
                    (parseInt([e.originScore, e.standardScore, e.percentScore, e.grade][i]) || 0),
                );
                if (e.grade) num[0] += 1;
              }
            }
          ...
          if (r.total) {
            ...
            r.total = {
              labels: [conv[key]],
              scores: [[sum[0]], [sum[1]], [sum[2]], [sum[3] / num[0]]],
            };
          if (r.total2) {
            r.total2.labels.push(conv[key]);
            r.total2.scores[3].push(sum2[3] / num[1]);
          } else
            r.total2 = {
              labels: [conv[key]],
              scores: [[null], [null], [null], [sum2[3] / num[1]]],
            };
          if (r.total3) {
            r.total3.labels.push(conv[key]);
            r.total3.scores[3].push(sum3[3] / num[2]);
          } else
            r.total3 = {
              labels: [conv[key]],
              scores: [[null], [null], [null], [sum3[3] / num[2]]],
            };
        });
    ...
    <>
      ...
      <div style={{width: '90%', margin: '0 auto'}}>
        <div className="mobile_menu" onClick={handleSwitch}>
          <button id="sum" className={left ? 'mobile_menuactive' : undefined}>
            조합별
          </button>
          <button id="individual" className={!left ? 'mobile_menuactive' : undefined}>
            과목별
          </button>
        </div>
        ...
        <div className="mobile_menu">
          <button
            className={scoreType == 1 ? 'mobile_menuactive' : ''}
            onClick={() => {
              setScoreType('1');
            }}
          >
          ...
          <button
            className={scoreType == 2 ? 'mobile_menuactive' : ''}
            onClick={() => {
              setScoreType('2');
            }}
          >
          ...
          <button
            className={scoreType == 3 ? 'mobile_menuactive' : ''}
            onClick={() => {
              setScoreType('3');
            }}
          >
...
export async function getStaticProps() {
  let {rows} = await pool.query(`select id, "type" , "year" , grade from "codeExams"`);
  const aa = await pool.query(
    `select id, "type" , "year" , grade from "codeExams" order by year, grade`,
  );
  const bb = await pool.query(
    `select code, "groupId" , "name" , description , "isUse" , sort from codes where "groupId" = 6`,
  );
  const cc = bb.rows.reduce((acc, row) => {
  ...
  return {
    props: {
      examcode: rows,
      subjectArea: bb.rows,
      conv: aa.rows.reduce((acc, obj) => {
        acc[obj['id']] = [obj.type, obj.grade, obj.year];
        return acc;
      }, {}),
    },
  };
