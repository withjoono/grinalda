...
const Linkage = ({personalcode}) => {
  ...
  const whatever = async () => {
    const res = await axios.get('/api/linkage', {
  ...
  const calculateTimeLeft = () => {
    ...
    let timeLeft = [];
    if (difference > 0) {
      timeLeft = [Math.floor((difference / 1000 / 60) % 60), Math.floor((difference / 1000) % 60)];
    }
    return timeLeft;
  };
  ...
  const link = async () => {
    try {
      const {data: res} = await axios.get('/api/linkage/check', {
        headers: {auth: localStorage.getItem('uid')},
        params: {random: input},
      });
      ...
      if (res.success == 'self') {
        alert('자신과 연동하실수 없습니다');
        return;
      } else if (res.success == 'overlap') {
        alert('잘못된 연동입니다');
        return;
      } else if (res.success == 'par') {
        alert('자녀만 연동 가능합니다');
        return;
      } else if (res.success == 'teach') {
        alert('학생만 연동 가능합니다');
        return;
      }

      setInfo([
        res.data.info.user_name || '미등록',
        personalcode[res.data.info.gradeCode] || '미등록',
        res.data.info.school || '미등록',
        res.data.info.hagwon || '미등록',
        res.data.mento_account,
      ]);
    } catch (e) {
      console.error(e);
    }
  ...
  const submitAccount = async e => {
    ...
    alert('연동되었습니다');
    await axios.get('/api/linkage/add', {
      headers: {auth: localStorage.getItem('uid')},
      params: {id: info[4], rot: router.query.groupid},
    });
  };
    ...
    <>
      ...
      <div className="content">
        ...
        >
          ...
          <div>
            2. 코드를 받은 멘티(학생)는 거북스쿨에 접속해서, 타계정연계란에 받은 코드를 입력합니다.{' '}
          </div>
            ...
            <div>
              ...
              {random != undefined ? (
                <span
                  ...
                  onClick={() => navigator.clipboard.writeText(random)}
                >
                  복사하기
                </span>
              ) : (
...
export async function getStaticProps() {
  const {rows} = await pool.query(`select code, "groupId" , "name" , description , "isUse" , sort
	 from codes where "groupId"= 12 and "isUse"=true`);
  const data = rows.reduce((acc, obj) => {
