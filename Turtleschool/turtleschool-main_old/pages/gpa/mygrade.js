...
import page from '../desktop/gradeanalysis';
    ...
    <>
      ...
      <div style={{width: '90%', margin: '0 auto'}}>
        ...
        <div className="mobile_menu">
          <button className={chosen == 0 ? 'mobile_menuactive' : ''}>
            {/*onClick={() => setChosen(0)*/}
            내신 평균 등급
          </button>
      ...
      <div
        style={{position: 'absolute', width: '100vw', backgroundColor: '#F5F5F5', padding: '5%'}}
      >
        {chosen == 0 ? (
          <>
            {' '}
            ...
            <div className="mobile_graph_menu" onClick={e => setGraph(e.target.value)}>
              <button
                value="individual"
                className={graph == 'individual' ? 'mobile_graph_menu_active' : ''}
              >
                과목별
              </button>
              <button value="sum" className={graph == 'sum' ? 'mobile_graph_menu_active' : ''}>
                조합별
              </button>
            </div>
            <div style={{position: 'relative', width: '100%'}}>
              ...
              <img src="https://img.ingipsy.com/assets/icons/arrow_down.svg" className="arrow" />
            </div>
            <HighchartsReact highcharts={Highcharts} options={chartOptions} />
          </>
        ) : (
