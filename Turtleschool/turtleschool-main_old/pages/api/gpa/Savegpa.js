  ...
  if (req.method == 'POST') {
    if (req.body.delete) {
      ...
      const {
        ...
        unit: req.body.unit,
        originscore: req.body.originscore,
        averagescore: req.body.averagescore,
        standarddeviation: req.body.standarddeviation,
      ...
      for (var i = 0; i < req.body.data.length; i++) {
        ...
        const {
          ...
          semester: req.body.data[i].semester,
          subjectarea: req.body.data[i].subjectarea,
          subjectcode: req.body.data[i].subjectcode,
          unit: req.body.data[i].unit,
          originscore: req.body.data[i].originscore,
          averagescore: req.body.data[i].averagescore,
          standarddeviation: req.body.data[i].standarddeviation,
          achievement: req.body.data[i].achievement,
          persons: req.body.data[i].persons,
          rank: req.body.data[i].rank,
          aper: req.body.data[i].aper,
          bper: req.body.data[i].bper,
          cper: req.body.data[i].cper,
    ...
    const data = await pool.query(
      `select id, grade , semester , subjectarea , subjectcode , unit , originscore ,
averagescore , standarddeviation , achievement , persons , "rank" , accountid , aper, bper , cper
 from gpa where accountid = $1
				`,
      [rows[0].id],
    );
    data.rows.sort((a, b) => {
