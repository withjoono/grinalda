...
export default async (req, res) => {
  ...
  if (req.method == 'POST') {
    for (var i = 0; i < req.body.data.length; i++) {
      ...
      await pool.query(
        `delete from csatmstrysprt where accountid = $1 and year = cast($2 as varchar) and useyn = 'Y' and recruitment = $3`,
        [req.headers.auth, req.body.data[i].year, req.body.data[i].recruitment],
      );
