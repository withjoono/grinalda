...
*/
  ...
  if (req.method == 'POST') {
    //1.저장전에 삭제
    await pool.query(`delete from csatidscore
            where 1=1
              and memberid = $1
              and year = $2
              and division = '1'`, [
    ...
    await pool.query(
      ...
      select $1, '1',
         subject_a, score_a, subject_b, score_b, standardscore,
          ...
          from (
              select split_part(score, ',', 1) as subject_a,
                     ...
                     cast(case when length(split_part(score, ',', 4)) > 0 then split_part(score, ',', 4) else null end as numeric) as percentage,
                	   ...
                	   cast(case when length(split_part(score, ',', 6)) > 0 then split_part(score, ',', 6) else null end as numeric) as subject_b,
                	   cast(case when length(split_part(score, ',', 7)) > 0 then split_part(score, ',', 7) else null end as numeric) as score_b
                from (
                    --'6F,,90,,|7F,,88,,|81,,,,1|1G,,,,2|10,,50,,|1F,,44,,'
                    --'6H,43,60,21,101,47,5|7F,49,70,13,118,62,4|81,76|1G,34,55,63,4|19,32,55,63,4|1A,43,55,63,4|9C,47'
