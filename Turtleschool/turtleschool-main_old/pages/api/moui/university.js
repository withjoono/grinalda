  ...
  switch (req.method) {
    case 'GET':
      const a = await pool.query(
        `select m.id, m.name, m.description , m."isUse" , m."areaCode" , m.maxscore , m.max, m.min , m.title_name
			 from universities u inner join "convertUniName" cun
			on u."name" = cun.변환 inner join mouidata m on m.대학 = cun.이투스 where u.id = $1`,
        [univ],
      );
