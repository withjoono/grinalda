  ...
  switch (req.method) {
    case 'GET':
      const a = await pool.query(
        `select max(u.id) as id, u."name",max(m.표점),min(m.표점), m.조합 from universities u inner join "convertUniName" cun
			on u."name" = cun.변환 inner join mouidata m on m.대학 = cun.이투스 where u."areaCode" = $1
			and (m.문이과 = $2 or coalesce ($2, '1') = '') group by u."name", m.조합 order by u."name" desc`,
        [area, ''],
      );
      ...
      if (z.rows.length > 0) {
        ...
        data = a.rows
          .map(r => getRange(dd.rows[0].get_combine_scores, r))
          .reduce((result, range) => {
            if (result.length == 0) result.push(range);
            else if (result[result.length - 1][2] == range[2]) {
              let prev = result.pop();
              result.push([
                Math.max(prev[0], range[0]),
                Math.min(prev[1], range[1]),
                range[2],
                range[3],
              ]);
            } else {
