...
export default async (req, res) => {
  let {rows} = await pool.query(`select account from members where account = $1`, [
    req.headers.auth,
  ]);
  ...
  await pool.query(
    `delete from adminclass a 
                        ...
                        a.memberid2=(select m.id::varchar from members m where m.account = $1)));`,
    [myid, id],
  );

  await pool.query(
    `delete from accountlinks
