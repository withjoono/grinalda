...
export default async (req, res) => {
  ...
  switch (req.method) {
    case 'GET':
      const {dvsn, cls, plnrid} = {dvsn: str_dvsn, cls: rows[0].cls, plnrid: rows[0].plnrid};
      ...
      rows = await pool.query(
        `
                        select
                            ...
                            avg(p2.score)        as score,
                            p1.cls          as cls,
                            RANK () OVER (partition by p2."primaryType" ORDER BY avg(p2.score)) as rrank,
                            p2."primaryType" as dvsn
                        from plannermanagement p1
                        left outer join planneritems p2 on p1.id = p2."memberId"
                                                       and to_char(p2."startDate", 'yyyy-mm-dd') between p1.strdt and coalesce(p1.enddt, '99991231') and p2."primaryType" = '학습'
                        ...
                        group by m1.id, m1.user_name, p1.cls, p2."primaryType"
                        order by score, id
                        ;
                `,
        [dvsn, cls, plnrid],
