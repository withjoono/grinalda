...
export default async (req, res) => {
  ...
  if (req.method == 'POST') {
    const {title, step, subject, startday, endday, type, material, amount, person, finished} =
      req.body;

    if (!req.body.id) {
      const a = await pool.query(
        `insert into plans (title,step,subject,range,type,material,total,person,done,memberid)
      ...
      const a = await pool.query(
        `update plans set (title,step,subject,range,type,material,total,person,done,memberid) =
 ...
 from plans where memberid = $1 and starttime is null`,
      [req.headers.auth],
    );
