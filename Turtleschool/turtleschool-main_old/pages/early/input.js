...
const EarlyInput = () => {
  ...
  const chooseData = () => {
    const rows = data
      .filter(row => row.grade == grade + 1 && row.semester == sem + 1)
      .map(row => {
  ...
  if (!page == 1) {
    return (
      <div className="page" style={{width: '90%', margin: '0 auto'}}>
        ...
        >
          <span style={{color: '#DE6B3D'}}>{localStorage.getItem('name')}</span>
          님의
          <br />
          내신 정보를 선택해 주세요.
        </div>
        <div style={{display: 'flex', margin: '12px -10px 12px 0'}}>
          <div
            className={grade == 0 ? 'buttonOn' : 'class'}
          ...
          <div
            className={grade == 1 ? 'buttonOn' : 'class'}
            style={{flex: '1 0 0', marginRight: '10px'}}
            onClick={() => setGrade(1)}
          >
            2학년
          ...
          <div
            className={grade == 2 ? 'buttonOn' : 'class'}
            style={{flex: '1 0 0', marginRight: '10px'}}
            onClick={() => setGrade(2)}
          >
            3학년
        ...
        <div
          className="orangebigbtn"
          onClick={() => {
            setChosen(chooseData());
            setPage(1);
          }}
        >
          입력하기
        ...
        <div className="notice">
          <span className="color">엑셀 파일로 내신 성적 입력을 원하시는 분은</span>
          <span>웹으로 접속하</span>
          <div>여 이용해주세요.</div>
          <br></br>
          <span className="color">교과 전형과 논술 전형</span>
          <span>은 단순 내신 평균을 이용하지만 (대학</span>
