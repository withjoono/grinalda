...
const Login = async e => {
  /*
  카카오 로그인 시작. 비동기적으로 authorize를 부르고 
  웹사이트 홈페이지로 리디렉트 되기 때문에 세션에 authorized를 저장함. 
  이 이후로는 _app.js 에서 세션에 authorized가 있을시 로그인 절차를 처리
  */
  const authorize = async () => {
    ...
    await Kakao.Auth.authorize({
      redirectUri: process.env.NEXT_PUBLIC_HOME_URL,
    }); //카카오가 리디렉트 시킴. 쿼리 스트링으로 code를 같이 넣음
  };
  e.preventDefault();
  authorize();
};
...
const googlelogin = async (res, login, user) => {
  ...
  if (res.user) {
    ...
    axios
      .get('/api/register', {
        headers: {auth: googleid},
        params: {type: 'google', access_token: at, name: name},
      })
      .then(res => {
        login[1](true);
        user[1](googleid);
        ...
        FB.api('/me', function (response) {
          ...
          axios
            ...
            .then(res => {
              ...
              localStorage.setItem('access_token', at);
              axios
                .get('/api/confirmlogin', {
                  params: {type: 'facebook', code: at},
                })
                .then(r => {
...
export {getId, refreshToken, googlelogin, facebooklogin, googleLogin2};
