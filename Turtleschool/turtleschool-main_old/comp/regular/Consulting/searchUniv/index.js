...
import { regionSearchUnivData,majorSearchUnivData,문이과구분 } from '../../../../common/univStandard/calc';
...
const SearchUniv = ({type, selectedUniv, onMajorClick, onMajorsClick,loginData,userScore,loading,setLoading, loadingBar }) => {
  ...
  const _getSearchUniv = () => {
    ...
    setUniv(regionSearchUnivData(문이과구분(userScore), type, selectedRegions));
    setLoading(false);
...
const UnivResult = ({type, result, setMajorResult, selectedRegions, setLineData,userScore,loading, setLoading, loadingBar}) => {
  ...
  const onMajorSearchClick = async({target}) => {
    ...
    const univName = target.id;
      ...
      var returnList = majorSearchUnivData(문이과구분(userScore),univName,type);
     


      if (returnList && returnList.length > 0) {
        await setLineData(prev => ({
          ...prev,
        ...
        await setMajorResult(returnList );
      }else {
...
const MajorResult = ({lineData,result,majorData,loading,setLoading, loadingBar}) => {
  const renderEmptyIndicator = () => (
    <S.EmptyContainer>
...
const SuitabilityResult = ({result,majorResult, selectedUniv, onMajorClick, onMajorsClick, calcMyScore,loading,setLoading, loadingBar}) => {
