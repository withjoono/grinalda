...
import SubjectSelect from '../../../../src/components/atoms/subjectSelect';
...
import {
  ...
  korCond,
  matCond,
} from '../../../../src/utils/inputConditions';
...
const AfterTestTable = ({onChangeIsScoreSaved, loginData}) => {
  ...
  const reducer = (state, action) => {
    switch (action.type) {
      case '데이터변경':
        return {
          ...state,
          [action.name]: {...state[action.name], [action.id]: action.value},
        };
    ...
    scores.forEach(score => {
      switch (score.lar_subject_nm) {
        case '국어':
          ...
          dispatch({
            name: 'kor',
            value: {standard: score.standardscore, percent: score.percentage, grade: score.grade},
          });
      ...
      axios
        ...
        .then(res => {
          ...
          if (res.data.success === true) {
            return alert('성적 저장에 성공했습니다.');
          } else {
            return alert('성적 저장에 실패했습니다. 재시도해주십시오.');
          }
        });
                  ...
                  return (
                    ...
                    >
                      <S.RegularInput
                        ...
                        min={0}
                        placeholder={
                          key === 'kor' || key === 'mat' || key === 'res1' || key === 'res2'
                            ? '[성적입력]'
                            : ''
                        }
                  ...
                  return (
                    ...
                    >
                      <S.RegularInput
                        style={{
                          color:
                            key === 'eng' || key === 'korHistory' || key === 'for'
                              ? 'black'
                              : 'blue',
                        }}
                        type="number"
                        ...
                        disabled={
                          key === 'eng' || key === 'korHistory' || key === 'for' ? true : false
                        }
                      />
                    </S.RegularTD>
                  );
                  ...
                  return (
                    ...
                    >
                      <S.RegularInput
                        ...
                        min={0}
                        placeholder={
                          key === 'eng' || key === 'korHistory' || key === 'for' ? '[성적입력]' : ''
                        }
                  ...
                  return (
                    ...
                    >
                      <S.RegularInput
                        ...
                        value={changeScoreFromOrgScore[key].grade}
                        onChange={changeData(key, 'grade')}
                        disabled={
                          key === 'kor' || key === 'mat' || key === 'res1' || key === 'res2'
                            ? true
                            : false
