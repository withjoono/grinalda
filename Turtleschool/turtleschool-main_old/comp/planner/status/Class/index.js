...
const Outline = ({pieData}) => {
  const weeklyStudyTime = useMemo(
    () => pieData.datasets[0].data.reduce((prev, curr) => prev + curr, 0),
    [pieData],
  );
...
const StatusList = ({
    ...
    <FormControl variant="outlined">
      <Select
        ...
        style={{
          width: 170,
          color:
            (type === '과목' && selectedSubject === '과목을 선택하세요.') ||
            (type === '계획' && selectedPlan === '계획을 선택하세요.')
              ? '#9A9A9A'
              : '#000000',
        }}
      >
        {[type === '과목' ? '과목을 선택하세요.' : '계획을 선택하세요.', ...labels].map(
          (label, index) => (
            <MenuItem
              key={label}
              style={{
  ...
  const renderEmptyStatus = () => (
    <S.StatusEmptyContainer>
      ...
      <S.StatusEmptyDescription>
        {'과목과 계획을 선택해 검색하면 해당 과목, 계획별 학습 현황을 확인할 수 있습니다.'}
      </S.StatusEmptyDescription>
    </S.StatusEmptyContainer>
  );
    ...
    const renderDropContent = status => (
      <S.StatusToggleContainer>
        ...
        <S.StatusToggleContent>
          ...
          <S.StatusToggleContentDescription>
            {status.score ? `${status.score}점` : '평가 전'}
          </S.StatusToggleContentDescription>
        </S.StatusToggleContent>
        <S.StatusToggleContent>
      ...
      <S.StatusContent>
        <S.StatusHeader>
          ...
          <S.StatusProgressContainer>
            ...
            <S.StatusProgress>{`시험결과 : ${
              status.score === null ? '없음' : `${status.score}점`
            }`}</S.StatusProgress>
          </S.StatusProgressContainer>
          <S.StatusButton onClick={onToggleClick}>
            ...
            <Image
              alt=''
              src={`https://img.ingipsy.com/assets/icons/arrow/${
                dropVisible[index] ? 'up' : 'bottom'
              }_grey_arrow.svg`}
              width={24}
              height={24}
            />
  ...
  const renderMentoInfo = () => (
    ...
    <S.MentoInfoContainer>
      <S.MentoInfoContent>
        <S.MentoInfoTitle>{'선생님'}</S.MentoInfoTitle>
        <S.MentoInfoDescription>{'강보리 선생님'}</S.MentoInfoDescription>
      </S.MentoInfoContent>
      <S.MentoInfoContent>
        <S.MentoInfoTitle>{'학습목표'}</S.MentoInfoTitle>
        <S.MentoInfoDescription>{'수업목표를 적어봅시다.'}</S.MentoInfoDescription>
      </S.MentoInfoContent>
    </S.MentoInfoContainer>
  );
  ...
  const onSearchClick = () => {
    _searchStatusData({
      subject: selectedSubject === '과목을 선택하세요.' ? '' : selectedSubject,
      title: selectedPlan === '계획을 선택하세요.' ? '' : selectedPlan,
    });
    ...
    <S.StatusContainer>
      ...
      <S.StatusFilterContainer>
        ...
        <S.StatusFilterContent>
          ...
          <S.StatusFilterInput>
            {renderSelectForm(
              plans.filter(plan => plan.subject === selectedSubject).map(plan => plan.title),
              '계획',
            )}
          </S.StatusFilterInput>
        </S.StatusFilterContent>
        <S.StatusFilterButton onClick={onSearchClick}>
          {'해당 과목 학습 현황 조회'}
      ...
      <S.StatusContentContainer>
        {statusData.length ? renderStatusContent() : renderEmptyStatus()}
      </S.StatusContentContainer>
    </S.StatusContainer>
  );
};
...
const Class = () => {
  ...
  const _searchStatusData = ({subject, title}) => {
    axios
      ...
      .then(res => {
        ...
        setDropVisible(res.data.data.map(() => false));
  ...
  return (
    <S.Container>
      ...
      <StatusList
        _searchStatusData={_searchStatusData}
        _getMontlyGraphData={_getMontlyGraphData}
        lineTestData={lineTestData}
        lineTaskData={lineTaskData}
...
WeeklySummary.propTypes = {
  lineTestData: PropTypes.node.isRequired,
  lineTaskData: PropTypes.node.isRequired,
};

StatusList.propTypes = {
  _searchStatusData: PropTypes.node.isRequired,
  _getMontlyGraphData: PropTypes.node.isRequired,
  statusData: PropTypes.node.isRequired,
  lineTestData: PropTypes.node.isRequired,
  lineTaskData: PropTypes.node.isRequired,
  dropVisible: PropTypes.node.isRequired,
  setDropVisible: PropTypes.node.isRequired,
  subjects: PropTypes.node.isRequired,
  plans: PropTypes.node.isRequired,
};
