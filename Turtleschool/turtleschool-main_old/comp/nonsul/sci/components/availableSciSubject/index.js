...
const AvailableSciSubject = ({
  ...
  const _getUniv = () => {
    ...
    const query = {
      ...
      mathd: 1,
      mathg: 1,
      scncc: filter.common || 1,
      scncpa: filter.physics1 || 1,
      scncca: filter.chemistry1 || 1,
      scncba: filter.biology1 || 1,
      scncea: filter.earth1 || 1,
      scncpb: filter.physics2 || 1,
      scnccb: filter.chemistry2 || 1,
      snccbb: filter.biology2 || 1,
      scnceb: filter.earth2 || 1,
      division: 1,
    ...
    axios
      ...
      .then(res => {
        ...
        const [pass, fail] = arr.reduce(
          ([p, f], univ) => (univ.gubu === '1' ? [[...p, univ], f] : [p, [...f, univ]]),
          [[], []],
        );
    ...
    <div id={cx('contents')}>
      <div id={cx('discuss')}>
        <div id={cx('discuss_contents')}>
          <Advice>
            * 과학을 출제하는 대학마다 범위가 상이합니다. <br></br>* 본인이 가능한 과학 과목을
            선택해주세요. <br></br>(주의) 연대와 건국대는 학과마다 과학 선택 과목이 다릅니다.{' '}
            <br></br>이 단계에서는 대학을 선택하시고, 학과를 선택하는 단계에서 학과별 과학 지정
            여부를 다시 확인해주세요
          </Advice>
          ...
          {/* <div className={cx('discuss_col1', 'section3')}>
                        <div className={cx('discuss_row2')}>
                            <p style={{ display: 'inline-block' }} className={cx('discuss_tit2')}>비추천 대학</p>
                            <Button onClick={selectAllAvoidUnivs}>
                                모두 선택
                            </Button>
                            <div className={cx('t_box', 't_bs')}>
                                <table className={cx('discuss_table2')}>
