...
const GpaSuperior = ({selectedUnivCode, gpaUnivCode, addUnivCodes, deleteUnivCodes}) => {
  ...
  const renderUnivItem = univ => [
    <tr key={univ.universityid + '_original_score'} className={cx('t_bt', 't_br', 't_bb', 't_bl')}>
      ...
      {univ.score.map((value, index) => (
        <td key={'score_' + index} className={cx({b_skyblue: gpaScore === index + 1})}>
          {value}
        </td>
      ))}
      <td rowSpan="2" className={cx('checkbox')}>
        <input
          checked={gpaUnivCode.includes(String(univ.universityid))}
          type="checkbox"
    ...
    <tr key={univ.universityid + 'diff'} className={cx('t_bt', 't_br', 't_bb', 't_bl')}>
      ...
      {univ.diff.map((value, index) => (
        <td key={'grade_' + index} className={cx({b_skyblue: gpaScore === index + 1})}>
          {value}
        </td>
      ))}
    </tr>,
      ...
      <div id={cx('discuss')}>
        <div id={cx('discuss_contents')}>
          <div className={cx('advice')}>
            * 논술전형에서 내신의 영향력은 크지 않지만, 본인 내신등급에 유리한 대학은 따로 있습니다.{' '}
            <br></br>* 표에서 보이는 대학 순서는, 각 대학별 내신 기준표를 가장 일반적인 내신 총점인
            300점 만점 <br></br>
            으로 통일하여, 본인 내신 등급에 유리한 순으로 나열한 것입니다(특허출원)<br></br>* 상위에
            랭크된 대학일수록 조금이라도 유리합니다.
          </div>
              ...
              <div className={cx('t_box', 't_bs')}>
                <div className={cx('input_container')}>
                  ...
                  >
                    ...
                    <input
                      className={cx('score_input')}
                      type="text"
                      id="num"
                      placeholder="등급을 입력 하세요"
                    ...
                    <tbody>
                      <tr className={cx('t_bg', 't_bt', 't_br', 't_bb', 't_bl')}>
                        ...
                        {Array.from({length: 9}, (_, i) => i + 1).map(index => (
                          <th
                            key={`${index}등급`}
                            className={cx({b_skyblue: gpaScore === index})}
                          >{`${index}등급`}</th>
                        ))}
                        <th>선택</th>
                      </tr>
