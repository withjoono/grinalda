    ...
    const renderUnivItem = (univ) => [
        <tr key={univ.universityid + '_original_score'} className={cx('t_bt', 't_br', 't_bb', 't_bl')}>
            ...
            {univ.score.map((value, index) => (
                <td key={index} className={cx({ b_skyblue: gpaScore === index + 1 })}>
                    {value}
                </td>
            ))}
        ...
        // <tr key={univ.universityid + '_k_score'} className={cx('t_bt', 't_br', 't_bb', 't_bl')}>
        //     <td className={cx('t_bg')}>1000점 만점</td>
        //     {univ.score.map((value, index) => (
        //         <td key={index} className={cx({ b_skyblue: gpaScore === index + 1 })}>
        //             {(value * (10 / 3)).toFixed(1)}
        //         </td>
        //     ))}
        // </tr>,
