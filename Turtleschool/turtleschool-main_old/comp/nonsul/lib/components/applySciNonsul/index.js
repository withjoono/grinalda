  ...
  const renderUnivItem = univ => (
    <tr key={`${univ.universityid},${univ.departmentid}`} className={cx('t_br', 't_bb', 't_bl')}>
      ...
      <td className={cx('checkbox')}>
        <input
          checked={firstSciUnivCode.includes(`${univ.universityid},${univ.departmentid}`)}
          type="checkbox"
        ...
        <div id={cx('discuss_contents')}>
          <div className={cx('discuss_col1', 'section1')}>
            <div className={cx('discuss_row1')}>
              * 이과논술 전형 중, 언어 논술은 보지 않고, 문과 범위의 수리논술만 출제하는 대학이
              있습니다. <br></br>* 수학에 자신 있고, 이과 대학으로 교차 지원을 희망할 경우, 아래
              대학을 선택해 주세요. 원치 않으실 경우 다음단계로 건너뛰세요.
            </div>
          </div>
