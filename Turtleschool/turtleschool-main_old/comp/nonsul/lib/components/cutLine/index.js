    ...
    axios
      ...
      .then(res => {
        ...
        const [pass, fail] = arr.reduce(
          ([p, f], univ) => (univ.pass_result_cd === 'Y' ? [[...p, univ], f] : [p, [...f, univ]]),
          [[], []],
        );
  ...
  const renderUnivItem = univ => (
    ...
    >
      ...
      <td className={styles.checkbox}>
        <input
          checked={selectedUnivCode.includes(`${univ.id},${univ.rcrtmunit}`)}
          type="checkbox"
          id={`${univ.id},${univ.rcrtmunit}`}
          name="선택"
      ...
      <div id={styles.discuss}>
        <div id={styles.discuss_contents}>
          <div className={styles.advice}>
            * 아래는 9월 모평 기준으로, 최저 가능 대학들과 불가 대학들입니다. <br></br> * 가능
            대학은 모두 선택 하세요. 불가 대학도 선택을 안 하시면, 뒷단계에서 배제되기 때문에,
            최저에 자신 있는 대학은 모두 선택해 주세요
          </div>
