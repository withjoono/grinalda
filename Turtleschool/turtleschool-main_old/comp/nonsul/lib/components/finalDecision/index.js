...
const FinalDecision = ({finalUnivs}) => {
  ...
  const _saveResult = async () => {
    await axios
      .post(
        '/api/essay/tab_save',
        {data: univ[0].map(univ => ({...univ, division: 0}))},
        {
        ...
        <div id={cx('discuss_contents')}>
          <div className={cx('discuss_col1', 'section1')}>
            <div className={cx('advice')}>
              * 지금까지 선택한 대학/학과를 전형 일자/시간이 겹치지 않도록, 전형일 순으로
              나열했습니다. <br></br>* 최저요건, 전형일 등을 다시 한번 꼼꼼히 확인하시고, 최적의
              조합이 나올 때까지 이전단계로 돌아가서 반복 해주세요.
            </div>
            <div className={cx('discuss_row2')}>
              <div style={{display: 'flex', flexDirection: 'row', justifyContent: 'space-between'}}>
                <p className={cx('discuss_tit2')}>최종 결정</p>
                <button onClick={_saveResult} className={cx('button_save')}>
                  저장하기
                </button>
              </div>
...
FinalDecision.propTypes = {
  finalUnivs: PropTypes.node.isRequired,
};
