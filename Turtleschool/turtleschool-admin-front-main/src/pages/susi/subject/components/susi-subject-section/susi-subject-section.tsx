...
import { SusiSubjectTable } from './susi-subject-table';
import { SuSiSubject } from '@/api2/types/susi-subject';
import { getAdminSusiSubjectListAPI } from '@/api2/수시_서비스/교과-api';
import { toast } from 'sonner';

export const SusiSubjectSection = () => {
  const [subjects, setSubjects] = useState<SuSiSubject[]>([]);
  ...
  useEffect(() => {
    const fetch = async () => {
      const result = await getAdminSusiSubjectListAPI({
        page,
        pageSize,
      });
      if (!result.success) {
  ...
  return (
    <>
      <SusiSubjectTable data={subjects} totalCount={totalCount} />
