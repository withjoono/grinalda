...
import { getAdminSusiComprehensiveAPI } from '@/api2/수시_서비스/학종-api';
import { SusiComprehensiveTable } from './susi-comprehensive-table';

export const SusiComprehensiveSection = () => {
  const [subjects, setSubjects] = useState<SusiComprehensive[]>([]);
  ...
  useEffect(() => {
    const fetch = async () => {
      const result = await getAdminSusiComprehensiveAPI({
        page,
        pageSize,
      });
      if (!result.success) {
  ...
  return (
    <>
      <SusiComprehensiveTable data={subjects} totalCount={totalCount} />
      <div className="flex items-center justify-end space-x-2 py-4">
        <div className="flex-1 text-sm text-muted-foreground">
          ...
          <div>
            아이템 - {(page - 1) * pageSize + subjects.length} / {totalCount}
          </div>
        </div>
        <div className="flex items-center space-x-4">
          ...
          <div className="space-x-2">
            ...
            <Button
              ...
              size="sm"
              onClick={() => setPage((prev) => prev + 1)}
              disabled={totalCount <= (page - 1) * pageSize + subjects.length}
            >
              다음
            </Button>
