...
import { SideLink } from '@/config/nav-config';
import { Button, buttonVariants } from './custom/button';

interface NavProps extends React.HTMLAttributes<HTMLDivElement> {
  isCollapsed: boolean; // 네비게이션이 접혀 있는지 여부
  links: SideLink[]; // 네비게이션 링크 배열
  closeNav: () => void; // 네비게이션을 닫는 함수
}

// Nav 컴포넌트 정의
export default function Nav({ links, isCollapsed, className, closeNav }: NavProps) {
  // 각 네비게이션 링크를 렌더링하는 함수
...
function NavLinkDropdown({ title, icon, label, sub, closeNav }: NavLinkProps) {
  ...
  // 하위 요소 중 하나가 활성화되어 있으면 Collapsible을 기본으로 열림
...
function NavLinkIconDropdown({ title, icon, label, sub }: NavLinkProps) {
  ...
  // 하위 요소 중 하나가 활성화되어 있으면 DropdownMenu를 기본으로 열림
  const isChildActive = !!sub?.find((s) => checkActiveNav(s.href));
