...
export const MockExamStandardForm = () => {
  ...
  //Queries
  const { data: standardScores, refetch: refreshStandardScores } =
  ...
  const initializeForm = (data: IMockExamStandardScore[]) => {
    ...
    const initialValues: FormSchemaType = {
      ko_score_standard: parseInt(ko_select_code?.standard_score || "0"),
      ko_score_grade: ko_select_code?.grade || 0,
      ko_score_percentile: ko_select_code?.percentile || 0,

      math_score_standard: parseInt(math_select_code?.standard_score || "0"),
      ...
      research_1_score_standard:
        science_select_codes.length > 0
          ? parseInt(science_select_codes[0]?.standard_score || "0")
          : parseInt(society_select_codes[0]?.standard_score || "0"),
      ...
      research_2_score_standard:
        science_select_codes.length > 1
          ? parseInt(science_select_codes[1]?.standard_score || "0")
          : society_select_codes.length > 1
            ? parseInt(society_select_codes[1]?.standard_score || "0")
            : parseInt(society_select_codes[0]?.standard_score || "0"),
      ...
      ko_select_code: ko_select_code?.subject_code || "S1",
      math_select_code: math_select_code?.subject_code || "S4",
      research_1_select_code:
        science_select_codes.length > 0
          ? science_select_codes[0]?.subject_code || "S10"
          : society_select_codes[0]?.subject_code || "S18",
      research_2_select_code:
        science_select_codes.length > 1
          ? science_select_codes[1]?.subject_code || "S11"
          : society_select_codes.length > 1
            ? society_select_codes[1]?.subject_code || "S20"
            : society_select_codes[0]?.subject_code || "S19",
      lang_select_code: lang_select_codes?.subject_code || "none",
    };
