...
import { cn, getRiskText } from "@/lib/utils";
import { Button } from "@/components/custom/button";
import ExcelExportButton from "@/components/custom/excel-export-button";
          ...
          <tbody className="font-semibold">
            {currentItems.map((admission) => (
              ...
              >
                ...
                <td className="break-all py-2">{admission.recruitment_name}</td>
                <td className="py-2">{admission.total_score}</td>
                <td className="py-2">
                  {admission.min_cut
                ...
                <td className="py-2">
                  {admission.errorMessage ||
                    admission.standardScore?.toFixed(2)}
                </td>
                <td
                  className={cn(
                    "py-2",
      ...
      <div className="flex justify-center">
        <ExcelExportButton
          data={admissions.map((item) => ({
            대학명: `${item.university.name}(${item.university.region})`,
            전형: item.admission_name,
            학과명: item.recruitment_name,
            총점: item.total_score,
            최초컷: item.min_cut ? parseFloat(item.min_cut).toFixed(2) : "-",
            추합컷: item.max_cut ? parseFloat(item.max_cut).toFixed(2) : "-",
            "내 점수": item.errorMessage || item.myScore?.toFixed(2),
            "동점수 평균": item.errorMessage || item.standardScore?.toFixed(2),
            점수차이: item.errorMessage || item.scoreDifference?.toFixed(2),
            "점수차이 (1000점 기준)":
              item.errorMessage || item.normalizedScoreDifference?.toFixed(2),
            위험도:
              item.errorMessage || (item.risk ? getRiskText(item.risk) : "-"),
          }))}
          filename="정시_전형_비교"
          buttonText="엑셀로 내보내기"
        />
