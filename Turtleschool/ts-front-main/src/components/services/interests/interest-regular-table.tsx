  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        ...
        <ExcelExportButton
          data={sortedData.map((admission) => ({
            대학명: `${admission.university.name}(${admission.university.region})`,
            전형: admission.admission_name,
            학과명: admission.recruitment_name,
            총점: admission.total_score,
            최초컷: admission.min_cut
              ? parseFloat(admission.min_cut).toFixed(2)
              : "-",
            추합컷: admission.max_cut
              ? parseFloat(admission.max_cut).toFixed(2)
              : "-",
            "내 점수": admission.errorMessage || admission.myScore?.toFixed(2),
            "동점수 평균":
              admission.errorMessage || admission.standardScore?.toFixed(2),
            점수차이:
            ...
            위험도:
              admission.errorMessage ||
              (admission.risk ? getRiskText(admission.risk) : "-"),
          }))}
          filename={`정시_전형_비교-${admissionType}군`}
          buttonText="엑셀로 내보내기"
        />
      </div>
