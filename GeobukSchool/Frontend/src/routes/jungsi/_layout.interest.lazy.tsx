...
function SusiInterests() {
  ...
  const classifySelectedItems = (items: IRegularAdmission[]) => {
    return items.reduce(
      (acc, item) => {
        if (item.admission_type === "가") acc.a.push(item);
        else if (item.admission_type === "나") acc.b.push(item);
        else if (item.admission_type === "다") acc.c.push(item);
        return acc;
      },
