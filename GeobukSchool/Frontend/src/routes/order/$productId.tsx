...
function ProductOrder() {
  ...
  const requestPay = async () => {
    ...
    const finalAmount = parseInt(product.data.product_price) - discountAmount;
    ...
    const preRegisterResult = await preRegisterPayment.mutateAsync({
      product_id: product.data.id,
      coupon_number: couponNumber.length ? couponNumber : undefined,
    });
    if (!preRegisterResult.success) {
      toast.error(preRegisterResult.error);
      return;
    }
    const merchantUID = preRegisterResult.data.merchant_uid;
  ...
  const handleCouponSubmit = async () => {
    ...
    const result = await validCoupon.mutateAsync({
      coupon_number: _couponNumber,
      product_id: product.data.id,
    });
    if (result.success) {
      toast.success("성공적으로 쿠폰을 등록했습니다.");
      setDiscountAmount(result.data.discount_price);
      setCouponInfo(result.data.coupon_info);
      ...
      <div className="w-full space-y-8 pt-4">
        <div className="w-full space-y-2 p-4 py-8">
          ...
          <div className="flex w-full items-center justify-between">
            <p className="text-xl">{product.data.product_nm}</p>
            <p className="text-xl">
              {formatPrice(Number(product.data.product_price))}
            </p>
          </div>
          ...
          <div className="space-y-2 pt-4">
            <p>{product.data.refund_policy}</p>
            <p>
        ...
        <div className="mx-auto w-full max-w-sm shrink-0 space-y-4 rounded-md border p-4">
          ...
          <div className="space-y-2">
            <div className="flex items-center justify-between">
              ...
              <p>
                {product.data.product_cate_code === "C"
                  ? "패키지"
                  : productTypeCodeName(product.data.product_type_code || "")}
              </p>
            </div>
            <div className="flex items-center justify-between">
              <p className="text-sm text-foreground/60">서비스기간</p>
              {product.data.product_type_code === "TICKET" ? (
                <p>1회</p>
              ) : (
            ...
            <div className="flex items-center justify-between">
              ...
              <p>{formatPrice(Number(product.data.product_price))}</p>
            </div>
            <div className="flex items-center justify-between">
            ...
            <div className="flex items-center justify-between">
              ...
              <p className="font-semibold text-primary">
                {formatPrice(
                  Number(product.data.product_price) - discountAmount,
                )}
              </p>
            </div>
          </div>
