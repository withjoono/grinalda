...
export const LifeRecordInputTabs: React.FC = () => {
  ...
  const mainSubjectList = useMemo(
    () => Object.values(staticData?.subjects.MAIN_SUBJECTS || {}),
    [staticData],
  );
  const subjectList = useMemo(
    () => Object.values(staticData?.subjects.SUBJECTS || {}),
    [staticData],
  );
  const {
    currentGrade,
  ...
  const onChangeFile = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file) {
      const result = await parsingSchoolRecord(file);

      console.log(result);
    }
  };

  const onClickParsingSchoolRecord = () => {
        ...
        <div className="w-full space-y-4 pt-4">
          ...
          <div className="flex justify-end gap-2">
            ...
            <Button
              variant="outline"
              type="button"
              onClick={onClickParsingSchoolRecord}
            >
