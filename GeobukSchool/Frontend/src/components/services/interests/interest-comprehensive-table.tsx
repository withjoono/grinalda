  ...
  ({
    ...
    return (
      ...
      >
        <ExcelExportButton
          data={data.map((item) => ({
            대학명: `${item.recruitmentUnit.university.name}(${item.recruitmentUnit.university.region})`,
            전형명: item.recruitmentUnit.admission.name,
            모집단위: item.recruitmentUnit.name,
            환산컷:
              parseFloat(item.recruitmentUnit.scores?.convert_50_cut + "") ||
              "-",
            추합컷:
              parseFloat(item.recruitmentUnit.scores?.convert_70_cut + "") ||
              "-",
            등급컷:
              parseFloat(item.recruitmentUnit.scores?.grade_50_cut + "") || "-",
            등급추합컷:
              parseFloat(item.recruitmentUnit.scores?.grade_70_cut + "") || "-",
            내등급: parseFloat(myGrade + "") || "-",
            계열적합성: getRiskText(item.compatibilityRisk),
            교과위험도: getRiskText(item.subjectRisk),
            비교과위험도: getRiskText(item.nonSubjectRisk),
            종합위험도: getRiskText(item.totalRisk),
          }))}
          filename="관심대학_목록_학종"
          sheetName="관심 대학 목록 (학종)"
          buttonText="엑셀로 내보내기"
        />
