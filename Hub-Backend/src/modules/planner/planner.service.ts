...
export class PlannerService {
  ...
  async getPlanners(memberId: number, dvsn?: string): Promise<any[]> {
    ...
    const query = `
      SELECT
        ...
        (SELECT COUNT(*) FROM pl_management p2
      ...
      FROM pl_class p
      JOIN auth_member m ON p.planner_id = m.id
      WHERE p.use_yn = 'Y'
  ...
  async getClassMembers(plannerId: number): Promise<any[]> {
    const query = `
      SELECT
        ...
        m.school
      FROM pl_management pm
      JOIN auth_member m ON pm.member_id = m.id
  ...
  async getPlannersMentor(memberId: number): Promise<PlannerMentorResponseDto[]> {
    const query = `
      ...
      FROM pl_management pm
      JOIN auth_member m ON pm.planner_id = m.id
      LEFT JOIN pl_class pc ON pc.planner_id = pm.planner_id
  ...
  async getRank(
    ...
    const rankQuery = `
      WITH achievement_scores AS (
        ...
        FROM pl_management pm
        LEFT JOIN pl_item pi ON pi.member_id = pm.member_id
          AND ${dateCondition}
        WHERE pm.use_yn = 'Y'
