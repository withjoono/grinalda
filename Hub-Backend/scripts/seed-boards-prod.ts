  ...
  try {
    ...
    for (const board of boards) {
      ...
      const existing = await client.query(
        `SELECT id, name, permission FROM board_board WHERE id = $1`,
        [board.id]
      );
      ...
      if (existing.rows.length === 0) {
        ...
        const result = await client.query(
          `INSERT INTO board_board (id, name, permission) VALUES ($1, $2, $3) RETURNING id, name, permission`,
    ...
    await client.query(`
      SELECT setval('board_tb_id_seq', (SELECT COALESCE(MAX(id), 0) + 1 FROM board_board), false)
    `);
    console.log('\n✅ Sequence reset to next available id');

    // Verify final state
    const allBoards = await client.query(
      `SELECT id, name, permission FROM board_board ORDER BY id`
    );
    console.log('\n📋 All boards:');
