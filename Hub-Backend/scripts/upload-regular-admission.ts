...
async function uploadRegularAdmission(excelFilePath: string) {
  ...
  try {
    ...
    await client.query('DELETE FROM js_admission_previous_result WHERE year = $1', [YEAR]);
    await client.query('DELETE FROM js_admission WHERE year = $1', [YEAR]);
    ...
    for (let i = 2; i < data.length; i++) {
      ...
      try {
        ...
        let universityResult = await client.query(
          'SELECT id FROM ss_university WHERE code = $1',
          [universityCode]
        );
        ...
        if (universityResult.rows.length === 0) {
          const insertResult = await client.query(
            `INSERT INTO ss_university (code, name, region, establishment_type)
        ...
        for (const result of previousResults) {
          await client.query(
            `INSERT INTO js_admission_previous_result
    ...
    const countResult = await client.query(
      'SELECT COUNT(*) as cnt FROM js_admission WHERE year = $1',
      [YEAR]
    );
