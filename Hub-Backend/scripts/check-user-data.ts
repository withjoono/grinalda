...
async function checkUserData() {
  ...
  try {
    ...
    const member = await client.query(
      `SELECT id, email FROM auth_member WHERE email = $1`,
      ['test@test.com']
    );
    ...
    const mockexam = await client.query(
      `SELECT COUNT(*) as cnt FROM js_sunung_standard_score WHERE member_id = $1`,
      [memberId]
    );
    ...
    const interests = await client.query(
      `SELECT COUNT(*) as cnt FROM auth_member_regular_interest WHERE member_id = $1`,
      [memberId]
    );
