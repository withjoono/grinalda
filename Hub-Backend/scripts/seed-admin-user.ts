...
async function seedAdminUser() {
  ...
  try {
    ...
    const existing = await client.query(
      `SELECT id, email, role_type FROM auth_member WHERE email = $1`,
      ['test@test.com']
    );
    ...
    if (existing.rows.length > 0) {
      ...
      if (user.role_type !== 'ROLE_ADMIN') {
        await client.query(
          `UPDATE auth_member SET role_type = 'ROLE_ADMIN', password = $1, update_dt = NOW() WHERE email = $2`,
        ...
        await client.query(
          `UPDATE auth_member SET password = $1, update_dt = NOW() WHERE email = $2`,
          [springCompatiblePassword, 'test@test.com']
        );
    ...
    const verified = await client.query(
      `SELECT id, email, nickname, role_type FROM auth_member WHERE email = $1`,
      ['test@test.com']
    );
